...
import { fetchActivePolicy, fetchPolicyOptions, fetchPublishFingerprint, publishSiddur } from "../api";
import { gatherSolverContext, servedOf } from "../applySolve";
import { calculateProfileScores, summarizePolicyScore } from "./profileScores";

export async function publishWithScores(departmentId: string, weekStart: string): Promise<string> {
  ...
  const policyScores = await Promise.all(policies.map(async (policy) => {
    ...
    const served = new Set(context.boardRides.filter((r) => r.status !== "cancelled").flatMap((r) => servedOf(r).map((s) => s.request_id!)));
  ...
  return publishSiddur(departmentId, weekStart, activeScores as unknown as Json, before, policyScores as unknown as Json);
}
