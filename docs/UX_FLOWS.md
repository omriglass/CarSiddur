...
**Wide screens (≥ lg)** — read-only `WeekGrid` (same component as the board, `readOnly`): rows = cars — shared cars always, a private (temporary) car only on days it has a ride (REQUIREMENTS §13.80, `features/siddur/visibleCars.ts`; 2026-09-14) — columns = 15-minute slots for the selected day; a week strip on top with 7 mini-columns to jump between days. Own rides are outlined; maintenance blocks hatched.
...
**Tablet/desktop layout.** A single day is shown at a time (a week of 15 cars at 15-minute resolution is 672 columns — unusable). Day tabs carry per-day unmet counts; a thin `WeekStrip` above the grid shows all 7 days as heat bars (rides / unmet) for orientation. Rows = cars (temporary cars in a separate group at the bottom — and, on the member siddur, only on days the private car has a ride, REQUIREMENTS §13.80; the Sadran board always lists every car; maintenance blocks hatched), columns = time 06:00–23:59 in 15-minute cells (major gridline every hour). Side panel on the start edge (right, in RTL) 320–360 px wide, collapsible: the `UnmetList`.
