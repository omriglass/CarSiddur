...
| Click block | `RideSheet`: details, passengers, pin toggle 🔒, boost with reason, split legs (when car not needed at destination), unassign, open request. Also lists any `ride_passengers` rows with a remove (×) where allowed, and a **"+ נוסעים"** button (any confirmed, uncancelled ride — not just a manual reservation) to append named passengers via `add_ride_passengers()` (REQ §13.85; same `AddPassengersDialog` the siddur's `RideDetailSheet` uses, §3.5). |
...
| `AddPassengersDialog` | `rideId, expectedVersion, departmentId, weekStart, disabled?` — the "+ נוסעים" button (REQ §13.85): members/children/free-text picker → `add_ride_passengers()`; self-contained (own queries + mutation), shared by `RideDetailSheet` (§3.5) and `RideSheet` (§4.2) (`src/features/siddur/components/AddPassengersDialog.tsx`; ✅ done 2026-09-14). |
| `RidePassengersList` | `expectedVersion, passengers, driverId?, canManageWeek?` — lists a ride's named `ride_passengers` rows with a remove (×) shown only where `remove_ride_passenger()` would allow it (`src/features/siddur/components/RidePassengersList.tsx`; ✅ done 2026-09-14). |
...
| `SharingTile` | `sharing: SharingStat` — S2 same-day sharing indicators, one card with three independent numbers (no combined score) plus muted raw-count subtitles (§5.12, `src/features/stats/components/SharingTile.tsx`; ✅ done 2026-09-14). |
