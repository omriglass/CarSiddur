...
| `newRequestButton.nextWeek` | בקשה לשבוע הבא | `NewRequestButton`, target week `open` |
...
| `newRequestButton.waitlistThisWeek` | רשימת המתנה לשבוע הזה | `NewRequestButton`, edge case: only the live week exists |
...
4. **Edge case (owner-confirmed 2026-09-14): only the `live` week exists, nothing newer open yet** → **"רשימת המתנה לשבוע הזה"** (`he.newRequestButton.waitlistThisWeek`), links to `/requests/new?week=<live>&waitlist=1`.
