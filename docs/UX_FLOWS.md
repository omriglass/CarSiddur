...
| `newRequestButton.nextWeek` | בקשה לשבוע הבא | `NewRequestButton`, target week `open`; also reused, disabled, when only the live week exists and nothing newer is open yet |
...
1. Target phase `open` → **"בקשה לשבוע הבא"** (`he.newRequestButton.nextWeek`), links to `/requests/new?week=<target>`.
...
4. **Edge case (owner-confirmed 2026-09-14, revised same day): only the `live` week exists, nothing newer open yet** → **disabled**, reuses state 1's own label **"בקשה לשבוע הבא"** (`he.newRequestButton.nextWeek`, `aria-disabled`, muted style like state 2) — the owner found a "this week" label confusing (pointing at next week on Sunday, then suddenly at this week on Thursday), so the button always reads as being about *next* week and simply disables itself when that week isn't open yet. Nothing to link to. `he.newRequestButton.waitlistThisWeek` was removed.
