...
My upcoming/ongoing rides are ordered chronologically by actual start time, with stable ride-ID ties. Every card shows Hebrew weekday, date, time window, actual destination and purpose; overnight rides also identify their ending date. The list includes every assigned leg of the member's requests, rides they drive without their own request (including volunteer driving), and rides where they are a named `ride_passengers.person_id` (REQ §13.82, 2026-09-14). Each ride appears once. Confirmed merged passengers appear on separate “מצטרף/ת X לY” lines (pickup legs use “מY”), excluding the member's own request. Missing-driver rides retain the red dashed presentation. The chosen week's requests and My Requests also show day/date/time/purpose and sort chronologically within each week.
...
**Wide screens (≥ lg)** — read-only `WeekGrid` (same component as the board, `readOnly`): rows = cars — shared cars always, a private (temporary) car only on days it has a ride (REQUIREMENTS §13.80, `components/weekGridCars.ts`, shared with the board; 2026-09-14) — columns = 15-minute slots for the selected day; **Sliding ride labels (owner, 2026-09-14):** the readable part of a ride block (badges, label, times) is a `position: sticky` wrapper offset by the header row, so on a long ride the text follows the reader down and stops at the block's own bottom edge; the block clips with `overflow-clip` (not `overflow-hidden`) so the grid viewport stays the scroll container sticky tracks. Same `WeekGrid`, so the board behaves alike; a week strip on top with 7 mini-columns to jump between days. Own rides are outlined; maintenance blocks hatched.
...
The main siddur has no destination/”where” filter. In both the phone list and wide grid, rides linked to the signed-in member's requests, assigned to them as driver, or where they are a named `ride_passengers.person_id` (REQ §13.82, 2026-09-14) use bold route/time text, a small star and “הנסיעה שלי”. Assigned personal rides also have an outline. Missing-driver bookings retain their red background and dashed red border; personal emphasis uses the bold text and star without replacing that status styling. Membership is based on request IDs, designated driver ID, or a named ride_passengers entry; this includes a passenger's one-way ride with no driver. Ride ordering stays chronological.
...
**Ride detail** (`RideDetailSheet`, a bottom sheet opened by tapping a `RideCard`/grid block — **not** a separate route; corrected 2026-09-09, no `/rides/:id` route exists in `src/app/router.tsx`/`memberRoutes`. A notification's `?ride=<id>` on `/siddur` opens this same sheet directly, see §2.1): header with times, origin → destination and car; car-mode indicator — "הרכב נשאר איתי ביעד" (keep), "הרכב נשאר בבנימינה" (relay out), "הרכב נאסף מבנימינה" (relay back), "הסעה — הנהג/ת חוזר/ת עם הרכב" (chauffeur); driver row (phone visible only if you share the ride, per §10); car; **ONE unified people list** (owner decision 2026-09-14, item 85 — `v_board_rides.people`, `RidePassengersList`/`ridePeople.ts`): driver first (marked "נהג/ת", never a remove ×), then every served request's requester/companions/named children/guest names, then anyone added directly via "+ נוסעים" below — all in one list, a directly-added person indistinguishable at a glance from a request-filed one. Each non-driver row gets a remove (×) when `remove_ride_person()`'s own gate allows it (any approved department member once the week is public, or a Sadran/admin who manages it — same gate the "+ נוסעים" button below uses; enforced server-side regardless of what the button shows). **There is no "ask to join" button here any more** (superseded, owner decision 2026-09-14, item 85/REQUIREMENTS §7.3): on a published/live week, "+ נוסעים" (`AddPassengersDialog`, disabled on a cancelled ride or before the week is public) is the way to join — its first control is a checkbox **"אני"** for the signed-in member themself (disabled, with a hint, if already on the ride), then the same picker as before — department members, department children, a free-text guest-names field (same parser as the request form's own guest field) — appended via `add_ride_passengers()`; the ride's driver is notified once per call naming who was added, unless the driver made the addition themself (REQUIREMENTS §13.85). The `join_ride_id`/proposal-based "ask to join" flow itself is unchanged and still reachable at `/requests/new?ride=<id>` (REQUIREMENTS §7.3, DATA_MODEL §3.6) — it is what the joinable-rides dialog used before 2026-09-14 (see §18) and what a shared/temporary-car merge proposal still runs on before a week is published. Buttons below the people list: **דווח/י על תקלה ברכב**, and for own rides **ערוך** / **בטל נסיעה**.
...
**Tablet/desktop layout.** A single day is shown at a time (a week of 15 cars at 15-minute resolution is 672 columns — unusable). Day tabs carry per-day unmet counts; a thin `WeekStrip` above the grid shows all 7 days as heat bars (rides / unmet) for orientation. Rows = cars (temporary cars in a separate group at the bottom — and only on days the private car has a ride — siddur and board alike, REQUIREMENTS §13.80, `components/weekGridCars.ts`; maintenance blocks hatched), columns = time 06:00–23:59 in 15-minute cells (major gridline every hour). Side panel on the start edge (right, in RTL) 320–360 px wide, collapsible: the `UnmetList`.
...
| Click block | `RideSheet`: details, passengers, pin toggle 🔒, boost with reason, split legs (when car not needed at destination), unassign, open request. Lists the same unified people list as the siddur's `RideDetailSheet` (§3.5 — `RidePassengersList` fed `v_board_rides.people`, driver first, remove × per `remove_ride_person()`'s own gate) and a **"+ נוסעים"** button (any confirmed, uncancelled ride — not just a manual reservation, and including the "אני" checkbox, §3.5) to append named passengers via `add_ride_passengers()` (REQ §13.85; same `AddPassengersDialog` the siddur uses). |
...
- **"Actions" kebab menu, every width.** Export the week (Excel), request deviations from the originally-filed times, **"שינוי מועד סגירת הבקשות"** (F2, 2026-09-14 — `SetWeekCloseAction`, hidden once the week is no longer `open`/`solving`: opens a `FormDialog` with a day picker bounded to `[open_at, publish_at]` plus a `TimeField15`, prefilled with the current `close_at` and a "ברירת המחדל של המחלקה: …" hint; confirms via `set_week_close_at`, which the Sadran may use to shorten *or* extend the window, owner A4), "השלם אוטומטית" (autofill still-unmet requests only), "פתור מחדש את כל השבוע" (full re-solve, shows its diff sheet before applying), cancel publication (reopen for requests, or unpublish back to solving — hidden while the week is `open` or `archived`, same as before), and links to the change log and the proposals list. The primary publish / close-and-publish button stays outside this menu, always visible in the header.
...
├──────────────────────────────────────────────────────────────┤
│ בחירת רכב:  (○) פיזור — מאזן קילומטראז' בין הרכבים            │
│             ( ) צפיפות — ממלא רכב אחד ומשאיר רכבים פנויים     │
├──────────────────────────────────────────────────────────────┤
...
- **Layout order**: header → date range → stat tiles (utilization, served rate, rides, people, policy score, cancellations) → effective-range line → sharing tile → busiest days → ride-type pie → requests-by-hour chart → weekly unmet chart.

**Statistics group S1-S4 (owner request, built 2026-09-14, `SharingTile`/`HourBarList`; `20260914110000_department_stats_sharing_indicators.sql`, docs/TODO.md "S — Statistics group")**:
- **S1** changes the underlying numbers of the existing שיעור ניצולת tile and busiest-days bar list only (each ride's occupied span now includes the department's turnaround buffer) — no new UI element.
- **ביטולים באותו יום** (`stats.cancellations`, a plain `StatTile` in the tile grid, hidden entirely when `cancellations` is absent from the RPC response): headline is the same-day cancellation rate (`cancellations.sameDayRate`, formatted like every other percentage tile) with sub-line "{{sameDay}} מתוך {{total}} ביטולים" (`stats.cancellations.sub`); when there were no cancellations at all in range the headline reads "אין ביטולים בטווח שנבחר" (`stats.cancellations.noCancellations`) instead of a misleading 0%. Caption is deliberately neutral (owner: cancelling is fine, cancelling at the last minute is the problem) — it never frames cancelling itself as bad.
- **Same-day sharing indicators** (`SharingTiles`, `src/features/stats/components/SharingTile.tsx`; hidden entirely when `sharing` is absent from the RPC response): **three plain `StatTile` blocks inside the main tile grid**, one headline number each, with their own help text — deliberately **no combined score** and, since the owner's follow-up the same day, no shared "שיתוף באותו יום" card (it read as one muddled figure). ניצול מקומות (`sharing.peopleUtilization`, a percentage — passenger-seats used ÷ seats available across the range's rides on shared cars); נסיעות לרכב ליום (`sharing.fragmentation`, one decimal place — average distinct rides per car per active day, always ≥ 1) with a muted subtitle "{{rides}} נסיעות מתוך {{days}} ימי רכב פעילים"; מענה לנסיעות בכיוון אחד (`sharing.oneWayFulfilment`, a percentage) with a muted subtitle "{{served}} מתוך {{total}}". A one-line footer spells out all three in one sentence (`stats.sharing.help`).
- **Requests-by-hour histogram** (`HourBarList`, `src/features/stats/components/HourBarList.tsx`, between the ride-type pie and the weekly unmet chart; hidden entirely when `requestsByHour` is missing/empty): a pure-CSS horizontal bar list, one row per hour 0..23 (Asia/Jerusalem), same approach as `WeekdayBarList` — bar width relative to the range's busiest hour, hour labels as `<span dir="ltr">08:00</span>`-style text, request count at the row's end. The quiet 00:00–05:59 range is hidden entirely when every one of those hours is at zero (rather than always showing six empty rows); it reappears the moment any night hour has a request. Title "בקשות לפי שעה ביום" (`stats.requestsByHour.title`).
...
| `notif.passengersAdded` | `outcome_changed` / `passengers_added` (variant) | "+ נוסעים": someone (not the driver) added named passengers to the driver's ride (REQ §13.85, 2026-09-14) | נוספו נוסעים לנסיעה שלך | {{byName}} הוסיף/ה את {{names}} לנסיעה שלך ביום {{day}} ל{{destination}} |
| `notif.passengerAddedYou` | `outcome_changed` / `passenger_added_you` (variant) | Unified ride-people model (REQ §13.85, 2026-09-14): a newly-added *member* (not the driver's aggregate notice above) is told directly | נוספת לנסיעה | {{byName}} הוסיף/ה אותך לנסיעה ל{{destination}} ביום {{day}} |
| `notif.passengersRemoved` | `outcome_changed` / `passengers_removed` (variant) | Unified ride-people model (REQ §13.85, 2026-09-14): the driver is told who was removed from their ride (`remove_ride_person()`), unless they removed them; never sent for a free-text guest | הוסרו נוסעים מהנסיעה שלך | {{byName}} הסיר/ה את {{names}} מהנסיעה שלך ביום {{day}} |
| `notif.passengerRemovedYou` | `outcome_changed` / `passenger_removed_you` (variant) | Unified ride-people model (REQ §13.85, 2026-09-14): the removed member is told directly, unless they removed themself | הוסרת מנסיעה | {{byName}} הסיר/ה אותך מהנסיעה ל{{destination}} ביום {{day}} |
| `notif.childRemoved` | `outcome_changed` / `child_removed` (variant) | Unified ride-people model (REQ §13.85, 2026-09-14): a removed child's guardian(s) (`child_guardians`) are told instead of the child, unless a guardian did the removing | ילד/ה הוסר/ה מנסיעה | {{byName}} הסיר/ה את {{childName}} מהנסיעה ל{{destination}} ביום {{day}} |
...
| `notif.window_changed` | `window_changed` | Sadran changed this week's request-closing time, shorter or longer (member; F2, `set_week_close_at` RPC, owner A2/A4, 2026-09-14) | מועד סגירת הבקשות השתנה | הבקשות לשבוע {{weekLabel}} נסגרות ביום {{closeDay}} בשעה {{closeTime}}. |
...
`outcome_changed` also has a `ride_cancelled` inbox/push variant (`_data.variant`, same selection mechanism as the `ride_change` variant of `proposal_received` above): sent to every other served passenger/driver — never the person who cancelled — and to any named `ride_passengers.person_id` (REQ §13.82, 2026-09-14) when a ride is fully cancelled (`cancel_ride_without_passengers()`, DATA_MODEL §3.10), before their request flips to `cancelled`. Title "{{byName}} ביטל/ה נסיעה שהיית בה", body "{{day}} {{depart}}–{{return}}, {{car}} ל{{destination}}." (`{{byName}}` is the person who cancelled, passed explicitly; the rest comes from `notification_context()` via `request_id`/`ride_id` in `data`, resolved while the ride/request rows are still intact).
...
| Screen | Text | Action |
|---|---|---|
| Home, no requests, week Open | עוד אין לך בקשות לשבוע {{weekLabel}}. הבקשות נסגרות ב{{closeTime}}. | `NewRequestButton variant="inline"` (F2: label/state depend on the *next* week, not necessarily this one — see §3.4 "F2 addendum") |
| Home, no requests, week Published | לא הגשת בקשות לשבוע הזה. אפשר להגיש בקשה חדשה — אם יש רכב פנוי היא תאושר מיד. | `NewRequestButton variant="inline"` |
...
| `PolicyChip` | `policyOptions: PolicyOption[]; activePolicy; value; stale; onSelect; scores?: Record<policyVersionId, number \| null>` — the board's policy chip + versions dialog (name, version, Asia/Jerusalem creation date, note, check mark on the current one), replacing the plain policy `<Select>` at every width (`src/features/sadran/board/components/PolicyChip.tsx`; ✅ done 2026-09-10, §4.2). **Live policy score (owner request 2026-09-14):** when `scores` has an entry for the selected policy, the chip label gains " · {{pct}}%" (rounded `alignment_ratio`, the same weighted-coverage figure `publishWithScores.ts` records at publish time, DATA_MODEL.md §3.9) with a tooltip explaining it, and each row in the dialog shows its own percentage (or "—" when not scored) — `scores` comes from `useBoardPolicyScores` (`src/features/sadran/board/useBoardPolicyScores.ts`, `policyScore.ts`), recomputed client-side from the board's own already-loaded data whenever the selected policy or the board's assignments change; purely informational, nothing is persisted or changes about solving/publishing. |
...
| `BoardActionsMenu` | `departmentId, weekStart, homeDestinationId, policy, onPolicyUsed, onAutoSolveRemaining, autoSolving` — board kebab menu, every width: export, request deviations, **change the week's closing time** (`SetWeekCloseAction`, F2 2026-09-14), auto-solve remaining, full re-solve, cancel publication, change-log/proposals links; hides items that don't apply, same as the old inline row (`src/features/sadran/board/components/BoardActionsMenu.tsx`; ✅ done 2026-09-10, §4.2). |
| `SetWeekCloseAction` | `departmentId, weekStart, renderTrigger?` — F2 (2026-09-14): "שינוי מועד סגירת הבקשות" kebab item; day picker (`DateField`, bounded to `[open_at, publish_at]`) + `TimeField15`, prefilled with `close_at`, department-default hint; `set_week_close_at` RPC; hidden once the week is no longer `open`/`solving` (`src/features/sadran/board/components/SetWeekCloseAction.tsx`). |
| `NewRequestButton` | `variant: "fab" \| "inline"` — F2 (2026-09-14): the shared three/four-state "בקשה חדשה" button (`newRequestButtonState()`), used by `AddRideFab` (fab) and Home's empty-week-requests action (inline) (`src/features/requests/components/NewRequestButton.tsx`). |
...
| `AddPassengersDialog` | `rideId, expectedVersion, departmentId, weekStart, people, disabled?` — the "+ נוסעים" button (REQ §13.85): a first **"אני"** checkbox (self; disabled with a hint when `people` already has this member), then the members/children/free-text picker → `add_ride_passengers()`; self-contained (own queries + mutation), shared by `RideDetailSheet` (§3.5) and `RideSheet` (§4.2) (`src/features/siddur/components/AddPassengersDialog.tsx`; ✅ done 2026-09-14, "אני" checkbox added 2026-09-14). |
| `RidePassengersList` | `rideId, expectedVersion, people, canManagePeople?, rideCancelled?` — ONE list of everyone on the ride (`src/features/siddur/ridePeople.ts`'s `RidePerson[]`, driver first, marked "נהג/ת", never removable), each other row with a remove (×) gated by `canRemoveRidePerson()` (the entry's own `removable` flag + `canManagePeople` + not `rideCancelled`); calls `remove_ride_person(rideId, expectedVersion, person.key)` for any key (`req:`/`comp:`/`child:`/`guest:`/`added:`), not only `added:<id>` (`src/features/siddur/components/RidePassengersList.tsx`; ✅ done 2026-09-14, repointed onto the unified `people` array 2026-09-14). |
...
| `SharingTiles` | `sharing: SharingStat` — S2 same-day sharing indicators as three plain `StatTile`s in the stats tile grid (ניצול מקומות / נסיעות לרכב ליום / מענה לנסיעות בכיוון אחד), each with its own help text; no combined score. |
| `HourBarList` | `hours: HourStat[]` — S4 requests-per-hour pure-CSS horizontal bar list, same approach as `WeekdayBarList`; hides the 00:00-05:59 range when it is entirely empty (§5.12, `src/features/stats/components/HourBarList.tsx`; ✅ done 2026-09-14). |
...
| `action.newRequest` | בקשה חדשה | superseded on the FAB/Home entry points by `newRequestButton.*` below (F2, 2026-09-14); key kept, unused elsewhere |
| `newRequestButton.nextWeek` | בקשה חדשה | `NewRequestButton`, target week `open`; also reused, disabled, when only the live week exists and nothing newer is open yet |
| `newRequestButton.preparing` | סידור בהכנה... | `NewRequestButton`, target week `solving` (disabled) |
| `newRequestButton.waitlistNextWeek` | רשימת המתנה לשבוע הבא | `NewRequestButton`, target week `published` |
...
| `weekClose.menuItem` | שינוי מועד סגירת הבקשות | board kebab "actions" menu item (F2, 2026-09-14) |
| `weekClose.dialogTitle` | מועד סגירת הבקשות | `SetWeekCloseAction`'s `FormDialog` |
| `weekClose.dialogDescription` | אפשר לקבוע מועד סגירה אחר לשבוע הזה בלבד — מוקדם או מאוחר יותר מברירת המחדל של המחלקה. | |
| `weekClose.dayLabel` / `weekClose.timeLabel` | יום סגירה / שעת סגירה | `DateField`/`TimeField15` labels |
| `weekClose.defaultHint` | ברירת המחדל של המחלקה: {{day}} {{time}} | `tv()` |
| `weekClose.confirm` | עדכון מועד הסגירה | dialog submit |
| `weekClose.saved` | מועד סגירת הבקשות עודכן | success toast |
| `weekClose.notEditable` / `weekClose.outOfRange` | אי אפשר לשנות את מועד הסגירה בשלב הזה של השבוע / מועד הסגירה חייב להיות בין פתיחת השבוע לפרסום שלו, ברבעי שעה עגולים | `src/lib/rpc.ts` error mapping for `week_close_not_editable`/`week_close_out_of_range` |
...
4. **The floating "+" button** on both הסידור and הבקשות שלי (`AddRideFab`, below) — as of F2 (2026-09-14) a **three/four-state** button (`NewRequestButton`, `src/features/requests/newRequestButton.ts`), not a fixed "בקשה חדשה" label linking unconditionally to `/requests/new`: see "Week-targeting rule" below for the states. "Take a car now" is the separate `CarNowButton` (item 3) on Home and in the siddur action row, unaffected.
...
**Joinable rides before the waiting list (F4, docs/TODO.md, owner 2026-09-14, REQ §13.83):** right after that toast, when the outcome is genuinely `waitlisted` (`shouldOfferJoinableRides()`, same field the toast branch above reads, so this only ever fires alongside it, never for open/solving-week silence or another outcome), `RequestForm.performSubmit` calls `joinable_rides_for_request(request_id)` (DATA_MODEL.md §7.7) before navigating away/closing the sheet. If it returns rows, `JoinableRidesDialog` (`src/features/requests/components/`) opens **instead of** the immediate navigate/`onDone`: title **"נכנסת לרשימת ההמתנה — אבל יש אפשרות נוספת"** ("you are on the waiting list, HOWEVER here is another option instead of just waiting"), body **"יש נסיעות באותו יום ליעדים קרובים (עד {{radius}} ק"מ). אפשר להצטרף ישירות לאחת מהן:"** (`department_settings.join_radius_km`), then one row per offered ride — time range, destination, driver name (or the car's name for a still-unclaimed ride), distance ("כ-{{km}} ק\"מ מהיעד שלך") and free seats (`he.siddur.freeSeats`). **Updated 2026-09-14 (owner decision, item 85):** each row's button, now **"הצטרפות לנסיעה"**, joins that ride directly instead of opening a second request form — `RequestForm`'s `joinNow()` re-reads the ride's current version (`fetchBoardRideById`, since `joinable_rides_for_request`'s own row carries none), calls `add_ride_passengers()` with the just-filed request's own people (the requester as an adult, its companions/children/guest names — captured in `joinRequestContext` at submit time, the same shapes `buildAddPassengerInputs` uses for the "+ נוסעים" dialog), then withdraws that now-redundant waitlisted request (`withdraw_request()`, its version re-read fresh via `fetchRequestVersion` rather than assumed) and toasts **"הצטרפת לנסיעה של {{driver}}"**. A failure (e.g. `ride_seats_exceeded`) surfaces its own error toast, leaves the dialog open and the waitlisted request untouched (withdraw only runs once the add has actually succeeded) — previously this button navigated to `paths.requests.new({ ride: rideId, week: weekStart })`, the same "ask to join" entry point §3.5's `RideDetailSheet` used to use before its own 2026-09-14 change. A secondary **"להישאר ברשימת ההמתנה"** button (or dismissing the dialog) simply continues the normal outcome (the deferred `onDone`/`navigate("/requests")`) unchanged. An empty result, or the lookup itself failing, is silent — the member sees exactly what they would have seen before this feature (the ordinary waitlisted toast, then navigate/close).

**WhatsApp quick link (owner amendment, 2026-09-14, REQ §10/§13.83):** next to each row's "הצטרפות לנסיעה", a WhatsApp icon button (`lucide-react`'s `MessageCircle`, reusing `buildWaUrl` from `src/features/sadran/proposals/waLink.ts`) opens `wa.me/<driver_phone>?text=<prefilled Hebrew>` in a new tab (`target="_blank" rel="noopener"`) — `he.joinableRides.whatsappText`: "היי {{driver}}, ראיתי שאת/ה נוסע/ת ל{{destination}} ביום {{day}} בשעה {{time}} — אפשר להצטרף לנסיעה?", filled from the row's own driver/car label, destination name, Jerusalem weekday (`weekdayLabel`) and start time (`formatTime`). Hidden when `driver_phone` is `null` (no phone on file, or the ride has no driver yet) — department phone numbers are not treated as secrets for this one dialog, but there is still nothing to link to. This is additive: the in-app "הצטרפות לנסיעה" button stays the primary, always-available action.
...
**Item 3 (`AddRideFab`, `src/features/requests/components/AddRideFab.tsx`):** the floating "+" button, previously duplicated (a plain `<a>` on `SiddurPage` and a `<Button asChild>` on `HomePage`, both always linking to `/requests/new`). Now one component, positioned with `fixed bottom-20 end-4 z-30 ... md:bottom-6` (logical `end-`, RTL-correct), used by both pages, that simply renders `<NewRequestButton variant="fab" />` (2026-09-10: the earlier live-week quick-sheet branch was removed — the FAB is a non-specific entry point; 2026-09-14/F2: it stopped being an always-on "בקשה חדשה" link and became the three/four-state button below).

**F2 addendum (2026-09-14, docs/TODO.md): the shared `NewRequestButton`.** `src/features/requests/components/NewRequestButton.tsx` (props `variant: "fab" | "inline"`) is the one place both `AddRideFab` (fab) and Home's empty-week-requests action (inline, replacing its old plain `<Button asChild><Link to="/requests/new">` pair) render from — the requests list page (`RequestsListPage`) has no "בקשה חדשה" button of its own to convert. Its label/link/enabled-ness is `newRequestButtonState()` (pure, `src/features/requests/newRequestButton.ts`, unit-tested), computed from the same `weeks` rows `resolveWeekStart` reads (`useWeeks`) — not a second source of truth. Let the **target week** be the newest non-archived week that is not `live` (the next week), falling back to the `live` week when nothing newer exists (a target whose phase is `upcoming` — materialized early only for a multi-day series leg, REQ §13.77, invisible to members — doesn't count as "next" either, so this falls back the same way):
1. Target phase `open` → **"בקשה חדשה"** (owner revision 2026-09-14: plain label; the button still refers to next week) (`he.newRequestButton.nextWeek`), links to `/requests/new?week=<target>`.
2. Target phase `solving` → disabled, **"סידור בהכנה..."** (`he.newRequestButton.preparing`, `aria-disabled`, muted style) — nothing to link to yet.
3. Target phase `published` → **"רשימת המתנה לשבוע הבא"** (`he.newRequestButton.waitlistNextWeek`), links to `/requests/new?week=<target>&waitlist=1` — the same explicit "waiting list" flow the siddur's own waitlist button already uses (REQ §13.66/consistency decision 24); `submit_request`/`try_auto_approve` still auto-approves it immediately if a car turns out to be free.
4. **Edge case (owner-confirmed 2026-09-14, revised same day): only the `live` week exists, nothing newer open yet** → **disabled**, reuses state 1's own label **"בקשה חדשה"** (`he.newRequestButton.nextWeek`, `aria-disabled`, muted style like state 2) — the owner found a "this week" label confusing (pointing at next week on Sunday, then suddenly at this week on Thursday), so the button always reads as being about *next* week and simply disables itself when that week isn't open yet. Nothing to link to. `he.newRequestButton.waitlistThisWeek` was removed.

Live-week quick requests (empty-slot click, `CarNowButton`) are unaffected (owner A3) — this button is only the non-specific entry point. `he.action.newRequest` ("בקשה חדשה") stays defined (nothing else used it once this landed) per the task's "keep the old key if anything else uses it".
...
**Week-targeting rule (REQUIREMENTS §13.74, revised 2026-09-10):** `RequestForm`'s own default week — when a request form is opened without an explicit `week` param (e.g. from a repeating-request suggestion's own week, or `/requests/new` bare) — resolves via `resolveWeekStart`: prefer the **open** week; if none is open, prefer the **next** week — the earliest week with `week_start` after today in phase `solving` or `published` (a late-request path once the current week has moved past `open`); only when no such next week exists does it fall back to **this** week (`live`, or a `solving`/`published` week whose `week_start` is today's — the Saturday just before it goes live). The waiting-list button and tapping a day cell on the siddur target the week being viewed, and `CarNowButton` always targets today. `resolveWeekStart.test.ts` covers open-alongside-live, published/solving "next" weeks ahead of the current live week, live-only, published-only (Saturday pre-rollover), archived/upcoming exclusion, and explicit-override precedence. **As of F2 (2026-09-14)** the FAB/Home entry point no longer relies on this fallback implicitly: `NewRequestButton`/`newRequestButtonState()` (addendum above) computes its own target week from the same `weeks` rows and always passes an explicit `?week=` — a distinct, narrower state machine (four states incl. disabled) built for a *label*, not just a default; `resolveWeekStart` keeps its original role for every other bare/no-week entry point.
...
- Coordinators can create a pinned free-text reservation by clicking an empty slot. It has no driver/request and displays its notes on both board views. Drag, resize, edit notes, and unassign use the existing ride flow. An optional people picker (members + their registered children) can name passengers on a new reservation; the first member picked becomes the driver (REQ §13.82, 2026-09-14). Named people see the ride under "my rides" and are notified when added (`outcome_changed` / `reservation_added` variant). An existing reservation's people can be edited via the `RideSheet` for draft/confirmed rides where `pin_reason = 'SADRAN_MANUAL'`; this never changes the pinned driver_id (fixed at creation).
