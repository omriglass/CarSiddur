...
#### `ride_passengers` (optional named people on a manual reservation, REQ §13.82, 2026-09-14)

Named department members and children added to a manual ride (not served by a request). Written only via `set_ride_passengers(ride_id, expected_version, passengers)` RPC. Reusable shared base for the future "+ נוסעים" feature.
...
| ride_passengers | `member_of(department_id) ∧ (is_week_public(department_id, week_start) ∨ can_manage_week(department_id, week_start) ∨ person_id = auth.uid())` | **RPC only** — `set_ride_passengers()` (Sadran/admin or the ride's own driver); no direct policy | **RPC only** — same RPC | **RPC only** — same RPC |
...
Ordered by distance, then `starts_at`, then `id`. `driver_name` is `''` for a still-unclaimed ride — never a phone number (§10, REQ §13.83); the contact channel is the existing "ask to join" flow (`join_ride_id`, §7.3 above), not this RPC.
...
Tests: `supabase/tests/joinable_rides.sql` — a within-radius ride with a free seat is returned; an otherwise-identical out-of-radius ride is excluded; the same ride on the next day is excluded; a full car (1-seat car, already carrying 1 adult) is excluded; a free-text (`destination_text`-only) sibling request returns no rows; another department member (neither the requester nor `can_manage_week`) is refused `not_authorized`; the Sadran (`can_manage_week`) can call it like the requester.
