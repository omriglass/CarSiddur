...
- **distinctPeople** / **distinctDrivers**: over the same ride set as `rides`/`activeHours` (every non-cancelled ride on a shared active car starting in range), `distinctPeople` is `count(distinct profile_id)` over the union of (a) `requester_id` of every request served by one of those rides (`ride_requests` join, `requests.status in ('assigned','merged')`), (b) `rides.driver_id` of those same rides (covers a chauffeur ride, which has no `driver` `ride_requests` row and so no served request of its own — the volunteer would otherwise be missed), and (c) `request_companions.profile_id` of those same served requests. A member with two served requests in range, or who both drove and rode along, counts once. Named children (`request_children`) and `guest_passenger_names` are **not** profiles and are **not** counted. `distinctDrivers = count(distinct rides.driver_id)` over the same ride set, reported as a sub-line.
