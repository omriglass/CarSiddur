...
Status: **DRAFT v0.3** (2026-09-06, owner answers applied; one-way/relay model, car location and the two Sadran events added; docs-consistency pass 2026-09-11, `docs/REFACTOR_PLAN_2026-09-11.md` C5; ride_passengers table and RPC added 2026-09-14; `add_ride_passengers`/`remove_ride_passenger` — the "+ נוסעים" button — added 2026-09-14; unified ride-people model — `v_board_rides.people`, `remove_ride_person()` replacing `remove_ride_passenger()`, `department_stats()` counting `ride_passengers` — added 2026-09-14). Derives from `docs/REQUIREMENTS.md` v0.3 (source of truth). Where this document and REQUIREMENTS disagree, REQUIREMENTS wins and this file must be fixed.
...
-- Canonical list = UX_FLOWS.md §6.1 (25 events). Value = snake_case of the i18n key suffix (`notif.freedSlotAuto` → 'freed_slot_auto').
create type public.notification_event   as enum ('window_open','window_closing','window_closed_solve_now','publish_reminder',
                                                 ...
                                                 'waitlist_contested','waitlist_resolved','window_changed');
...
- `window_changed` (F2, REQ §13.81, 2026-09-14) is a **member** event (mutable, no variants) sent to every approved department member whenever a Sadran changes a week's `close_at` via `set_week_close_at()` (§3.11), whether shortening or extending it — regardless of any resulting phase flip.
...
| join_radius_km | numeric(5,1) | NN | 10 | CHECK `join_radius_km >= 0 and join_radius_km <= 100`; F4 (docs/TODO.md, owner 2026-09-14, REQ §13.83) — how close a ride's destination must be to a waitlisted request's own (preset) destination to be offered as a "joinable ride" instead of just waiting (`joinable_rides_for_request()`, §7 below), admin-editable |
...
| settings | jsonb | NN | `{}` | Car-choice option (owner, 2026-09-14; `SOLVER.md` §3.6/§3.6.2, REQUIREMENTS §13.84): `{ "carChoice": "pack" \| "spread" }`. CHECK `jsonb_typeof(settings) = 'object'` and `coalesce(settings->>'carChoice', 'spread') in ('pack', 'spread')`. Absent key / `{}` (every version predating this column) = `'spread'`, today's behaviour. |
| note | text | | | change note |
| created_by | uuid | NN | | |
| created_at | timestamptz | NN | now() | |

Trigger `forbid_mutation()` raises on UPDATE/DELETE: "Changing a policy never rewrites history". A solver run references a `policy_version_id`, never a policy. `create_policy_version(p_policy_id, p_rules, p_note default null, p_settings default '{}')` (`SECURITY DEFINER`, `can_manage_operations(department_id)`) inserts the new immutable version and repoints `policies.current_version_id`; `src/features/solverBridge/buildSolverInput.ts`'s callers read `settings.carChoice` off the active/chosen version and set it on the solver's `Policy.carChoice`.
...
#### `ride_passengers` (named people on a ride with no request behind them, REQ §13.82/§13.85, 2026-09-14)

Named department members and children (or a free-text guest) added to a ride — either a Sadran's manual reservation (`set_ride_passengers`, REQ §13.82) or, additively, any published ride via the "+ נוסעים" button (`add_ride_passengers`/`remove_ride_passenger`, REQ §13.85).
...
| ride_id | uuid | NN | | FK rides ON DELETE CASCADE |
| department_id / week_start | uuid / date | NN | | composite FK weeks |
| person_id | uuid | | | FK profiles; one of person_id or child_id is set, not both |
| child_id | uuid | | | FK children; one of person_id or child_id is set, not both; CHECK `ride_passengers_one_link_ck` ensures at most one |
| display_name | text | NN | | shown on board/siddur |
| seat_kind | text | NN | | CHECK in `(‘adult’,’child_seat’,’booster’)`; seat fit: deferred trigger `ride_seat_fit_check()` (§5.2) counts these rows alongside `ride_requests` |
| added_by | uuid | NN | | FK profiles — who added them |
| created_at / updated_at | timestamptz | NN | now() | `set_updated_at()`, `audit_row()` |

Indexes: partial unique `(ride_id, person_id) where person_id is not null` and `(ride_id, child_id) where child_id is not null` (at most one row per person/child per ride); plain `(ride_id)` and `(person_id, week_start)` (the latter for "fetch my rides including where I’m named a passenger"). Triggers: `set_updated_at`, `audit_row`. **RPC `set_ride_passengers(p_ride_id uuid, p_expected_version int, p_passengers jsonb)`** (SECURITY DEFINER, `grant execute … to authenticated`, 2026-09-14): replaces the ride’s entire `ride_passengers` set in one transaction. `p_passengers` is `[{person_id?, child_id?, display_name, seat_kind}]`; validation: each person must be an approved department member, each child must belong to the department, exactly one of person_id/child_id is set, display_name required, seat fit via `car_fits()` against the car’s seat configuration minus existing `ride_requests` load. Callable by `can_manage_week(ride.department_id, ride.week_start)` (Sadran/admin) or the ride’s own `driver_id`. Bumps the ride’s `version` (via `update rides set updated_at = now()`). Enqueues ONE `outcome_changed` notification per newly-added `person_id` (not for children, not for removed ones, not for the caller) with variant `reservation_added`, using the EXISTING `outcome_changed` event so `notification_default_url()` resolves the ride deep link correctly (§3.11, ARCHITECTURE §9). `v_board_rides.passengers` view column carries the JSON list (`id, person_id, child_id, display_name, seat_kind, added_by`) for display on the board and siddur — kept for backward compatibility alongside the unified `people` column below.

**RPC `add_ride_passengers(p_ride_id uuid, p_expected_version int, p_passengers jsonb)`** (SECURITY DEFINER, `grant execute … to authenticated`, 2026-09-14/2026-09-14 unified pass; REQ §13.85): the "+ נוסעים" button's write path, same row shape as `set_ride_passengers()` but **appends** instead of replacing, and never touches `driver_id`. Callable by any `member_of(ride.department_id)` (`not_authorized` otherwise) once `is_week_public(department_id, week_start)` (or the caller already `can_manage_week`; `ride_week_not_public` otherwise); refuses a cancelled ride or an archived week (`ride_not_found`/`week_archived`). Validates every row the same way as `set_ride_passengers()`, then silently **skips** (no error) a row already present — as an existing `ride_passengers` person/child on this ride, or already served on this ride via a request's `requester_id`/`request_companions`/`request_children` — before checking seat capacity (`car_fits()` against served `ride_requests` load + existing `ride_passengers` + the newly-accepted rows; `ride_seats_exceeded` otherwise) and inserting. Enqueues ONE `outcome_changed` / `passengers_added` notification to the ride's driver per call (not per row) — vars `byName` (the acting member) and `names` (a comma-joined list of every row actually inserted this call) are passed explicitly since this call has no `request_id` for `notification_context()` to derive a `destination` from; `destination` is likewise passed explicitly (from `ride.destination_id`), while `day` is filled for free from the ride's own `starts_at`. Skipped when the driver made the addition themself. **(2026-09-14 unified pass, `20260914190000_unified_ride_people.sql`)**: additionally enqueues ONE `outcome_changed` / `passenger_added_you` notification to each newly-added *member* (`person_id`, not a child, not a guest), unless that member added themself — the same vars (`byName`, `destination`) as the driver's notice.

**RPC `remove_ride_person(p_ride_id uuid, p_expected_version int, p_key text)`** (SECURITY DEFINER, `grant execute … to authenticated`, 2026-09-14 unified pass, `20260914190000_unified_ride_people.sql`; REQ §13.85; **replaces** the narrower `remove_ride_passenger(p_ride_passenger_id uuid, p_expected_version int)`, which took only a `ride_passengers.id` and was refused to anyone but the row's adder/named person/driver/manager — dropped in the same migration): removes exactly one person from the ride's unified `people` list (below), addressed by that entry's `key`. Same authorization gate as `add_ride_passengers()` — any `member_of` once the week is public/managed (`not_authorized`/`ride_week_not_public`), never a cancelled ride or archived week — a deliberate broadening of the old RPC's four-role check (owner decision 2026-09-14, rule 1: "anyone may add or remove anyone"). Per key prefix:
- `driver:<profile_id>` — always refused, `ride_driver_not_removable` (cancelling the ride is the path).
- `added:<ride_passenger_id>` — deletes the `ride_passengers` row.
- `comp:<request_id>:<profile_id>` — deletes the `request_companions` row and bumps that request's `version`.
- `child:<request_id>:<child_id>` — deletes the `request_children` row and bumps that request's `version`.
- `guest:<request_id>:<n>` (1-based) — drops the *n*-th name from that request's `guest_passenger_names` array.
- `req:<request_id>` — a non-driver requester's participation in *this* ride only: deletes the `ride_requests` row and marks the request `withdrawn` (`status_reason = 'WITHDRAWN_BY_MEMBER'`); the ride and its other passengers are unaffected. (`req:<driver's own request>` is refused the same as `driver:`, defensively.)

Bumps the ride's `version` (`update rides set updated_at = now()`) in every case. Notifications (`outcome_changed`), unless skipped for a free-text guest (`added:`/`guest:` with no `person_id`/`child_id` — **nobody** is notified, not even the driver): the driver is told who was removed (`passengers_removed`, `names` = the removed display name; skipped if the driver did the removing), the removed member is told too (`passenger_removed_you`, skipped if they removed themself; not applicable to a `guest:`/child removal), and a removed child's `child_guardians` are told instead (`child_removed`, `childName` = the child's name; skipped for a guardian who did the removing).

`v_board_rides.people jsonb` (added 2026-09-14, `20260914190000_unified_ride_people.sql`, wrap-and-append like `.passengers` above): one entry per person on the ride — driver, every served request's non-driver requester, its companions/children/guest names, and every `ride_passengers` row — ordered driver first, then `display_name`, then `key`. Shape: `{ key text, source text in ('driver','requester','companion','child','guest','added'), request_id uuid|null, ride_passenger_id uuid|null, person_id uuid|null, child_id uuid|null, display_name text, seat_kind text in ('adult','child_seat','booster'), added_by uuid|null, removable boolean }` — `removable` is `false` only for the `driver` entry (the RPCs above still re-check authorization/role regardless of this flag; it exists only so the UI can decide whether to render a remove affordance at all). `seat_kind` for a `requester`/`companion`/`guest` entry is always `'adult'` and for a `child` entry always `'child_seat'` — neither `request_companions` nor `request_children`/`guest_passenger_names` stores a real per-person seat category (that capacity math still runs on `requests.adults/child_seats/boosters` and `ride_passengers.seat_kind`, both unchanged); only an `added` entry's `seat_kind` reflects a real stored value. `department_stats()`'s `distinctPeople` and `sharing.peopleUtilization` (§7.6) include `ride_passengers` rows on the same basis as a served request's requester/companion, so a directly-added person is not invisible to statistics.
...
**Live board preview (owner, 2026-09-14):** the Sadran board also previews each policy's `alignment_ratio` ("weighted coverage") client-side, next to and inside the policy chip's versions dialog (`src/features/sadran/board/policyScore.ts`, `useBoardPolicyScores.ts`, UX_FLOWS.md §4.2 `PolicyChip`) — computed the same way `publishWithScores.ts` does, from data the board already has loaded; nothing is stored until an actual publish writes `siddur_versions.snapshot.policy_scores`.
...
**`set_week_close_at(p_department_id uuid, p_week_start date, p_close_at timestamptz) returns void`** (F2, REQ §13.81, `20260914100200_set_week_close_at.sql`; SECURITY DEFINER, `grant execute … to authenticated`) — the Sadran-only "change this week's request-closing time" action behind the board's kebab menu (`SetWeekCloseAction`). Requires `can_manage_week(department_id, week_start)`; the target week's `phase` must be `open` or `solving` (else `week_close_not_editable`); `p_close_at` must be strictly after `open_at`, no later than `publish_at`, and on the 15-minute grid (`is_quarter_hour`), else `week_close_out_of_range`. Updates `close_at` (audited via `app.audit_reason = 'set_week_close_at'`, same convention as `reopen_week`); if the new `close_at` is already in the past while `phase = 'open'`, flips it to `solving` and sends the department's Sadranim the same `window_closed_solve_now` notice `advance_week_phases()`'s own open→solving branch sends (dedupe key folds in `p_close_at` so a later *natural* close via `app.tick()` is never deduped against this manual one); if the new `close_at` is in the future while `phase = 'solving'`, flips it back to `open` (the only place this reverse transition exists — `advance_week_phases()` never runs it). Always enqueues one `window_changed` notification (§2, §3.11) to every approved department member with `closeDay`/`closeTime` vars (`weekday_short_label()`/`to_char(... , 'HH24:MI')`); `{{weekLabel}}` comes from `notification_context()`, which reads `weeks.close_at` fresh — i.e. *after* this RPC's own `update`. `advance_week_phases()` needs no change: it already reads `close_at` off the row on every tick, so a Sadran override (earlier or later) is honored automatically. Migrations: `20260914100100_notification_event_window_changed.sql` (`alter type notification_event add value 'window_changed'`, alone per house rule) then `20260914100200_set_week_close_at.sql` (the RPC above, `notification_default_url()`'s updated body, and the seeded `window_changed` template rows — also mirrored in `supabase/seed.sql`).
...
| ride_passengers | `member_of(department_id) ∧ (is_week_public(department_id, week_start) ∨ can_manage_week(department_id, week_start) ∨ person_id = auth.uid())` | **RPC only** — `set_ride_passengers()` (Sadran/admin or the ride's own driver) or `add_ride_passengers()` (any `member_of` once the week is public/managed); no direct policy | **RPC only** — same RPCs | **RPC only** — `set_ride_passengers()` (replace; Sadran/admin or driver) or `remove_ride_person()` (any `member_of` once the week is public/managed, keyed by `people[].key`, 2026-09-14 — replaced the narrower `remove_ride_passenger()`) |
...
- `notification_templates`: one `inbox` + one `push` row per `notification_event` ((historical, superseded — 24 events as of 2026-09-10) 22 events, since `car_care` was added 2026-09-09 — §6.1's "Car care portal" narrative subsection below) and 5 `whatsapp` variants (`shift`, `merge_passenger`, `merge_driver`, `deny`, `reminder`), copied from UX_FLOWS §6.
...
### Bug-fix migrations 2026-09-07 → 09-09 not otherwise cited

Audit pass, `docs/REFACTOR_PLAN_2026-09-11.md` C5. The 20 migrations below (checked by grepping every `20260907`/`20260908`/`20260909` filename against this whole document) are not named anywhere else in DATA_MODEL.md; each is additive, one concern per file, and not individually load-bearing enough to warrant its own numbered item above. Gloss taken from each file's own header comment, not invented:

| Migration | Gloss |
|---|---|
| `20260907093400_enable_sadran_operations.sql` | Operational administration for Sadranim; identity/department authority stays unchanged. |
| `20260907093700_fix_notification_interpolation.sql` | Notification interpolation uses persisted context, with no auto-approval fan-out. |
| `20260907094000_fix_accepted_board_proposals.sql` | Accepted board shifts reuse rides; merges release previous solo bookings. |
| `20260907094200_order_ride_edit_authorization.sql` | Authorize existing own ride before checking member-only time rules. |
| `20260907094300_fix_bulk_request_withdrawal.sql` | Remove PL/pgSQL/table alias ambiguity; allow score aggregation rounding. |
| `20260907094400_cancel_pending_ride_changes.sql` | Pending shadow requests can be withdrawn without touching confirmed rides. |
| `20260907094500_fix_merge_leg_alias.sql` | Qualify target leg JSON separately from `ride_requests.leg`. |
| `20260907094600_audit_ride_change_consent.sql` | Version and audit consent changes consistently with the other state tables. |
| `20260907094900_allow_coordinator_tight_turnarounds.sql` | Coordinators may explicitly shorten preparation gaps; occupied ride times never overlap. |
| `20260907095000_preserve_passengers_without_driver.sql` | Persistent passenger bookings can await a volunteer driver without losing their slot. |
| `20260907095500_preserve_manual_gaps_when_applying_shifts.sql` | Accepted coordinator shifts use the same bounded preparation-gap exception as board edits. |
| `20260907095900_edit_ride_public_notes.sql` | Edit public ride information independently of scheduling/consent. |
| `20260907100000_car_access_and_replacement_codes.sql` | Vehicle access codes retain leading zeroes; existing cars have no stored code. |
| `20260907103000_default_board_start_at_six.sql` | Start the visible board at 06:00; the existing early-hours toggle reveals earlier rides. |
| `20260907104000_complete_day_readiness_and_legacy_reopen.sql` | Readiness needs every requested leg, not merely any assignment. |
| `20260908120900_add_status_changed_notification.sql` | `alter type notification_event add value 'status_changed'` (alone, per house rule). |
| `20260908122000_week_opening.sql` | Catch up missed openings without reopening closed or published weeks. |
| `20260908142000_protect_last_admin.sql` | Keep an approved administrator available to administer the system, enforced at the data boundary. |
| `20260908154000_fairness_by_granted_hours.sql` | Fairness is based solely on time already granted, never on how many times a member asked. |
| `20260909091000_notify_proposal_creator_on_answer.sql` | `proposal_answered` must reach the Sadran who actually sent the proposal, not every Sadran of the week. |
...
### 7.3a Per-car mileage totals for the solver — F5, REQ §13.84, SOLVER.md §3.6.2
Rolling **4 weeks** before the target week, fixed in v1 (no department setting), plus the target week's own already-`confirmed` rides (never that week's own not-yet-applied draft — a solve in progress must not count itself). Distance is read off the ride's **served request** (`ride_requests` → `requests.destination_id`, preferring the `driver` row, falling back to any served request for a driverless chauffeur leg — `rides.origin_id`/`destination_id` are almost always home, §3.7 `rides`, so the far point lives on the request, not the ride); unknown/free-text (`destination_id is null`) contributes `0`.
```sql
-- $1 department_id, $2 target week_start, $3 weeks back (default 4)
select c.id as car_id,
  coalesce(sum(
    coalesce(dest.distance_km, 0) * case when r.origin_id = r.destination_id then 2 else 1 end
  ), 0) as km
from cars c
left join rides r
  on r.car_id = c.id and r.status <> 'cancelled'
 and r.week_start >= $2 - ($3 * 7) and r.week_start <= $2
 and (r.week_start < $2 or r.status = 'confirmed')
left join lateral (
  select rq.request_id from ride_requests rq where rq.ride_id = r.id
  order by case when rq.role = 'driver' then 0 else 1 end, rq.request_id limit 1
) served on true
left join requests req on req.id = served.request_id
left join destinations dest on dest.id = req.destination_id
where c.department_id = $1 and c.type = 'shared' and c.status = 'active'
group by c.id;
```
Exposed as `car_mileage_totals(p_department_id, p_week_start, p_weeks default 4)` (`20260914140000_car_mileage_totals.sql`, definer). Unlike `fairness_stats`, callable by **any** department member (`member_of()`), not just the Sadran/admin — mileage is not sensitive. Consumed by `buildSolverInput`'s `mileageKmByCarId` → `Car.mileageKm`, the input to the mileage-balancing tie-break (SOLVER.md §3.6.2): a round trip (`origin_id = destination_id`, parked home both ends) counts double, a relay leg (`origin_id ≠ destination_id`) counts once (its partner leg is its own ride row, already counted once).
...
`department_stats(p_department_id uuid, p_from date, p_to date) returns jsonb` (`stable security definer set search_path = public, pg_temp`, `20260910097000_add_department_stats.sql`, extended in place by `20260910098100_add_department_stats_earliest.sql` and `20260910098300_extend_department_stats_metrics.sql`), granted to `authenticated` only. Authorization: `is_admin() or is_sadran_any(p_department_id)`, else `not_authorized` (`P0001`) — the same helper vocabulary as every other RLS-adjacent check, not a new one. Rejects `p_to < p_from` or a span over 400 inclusive days with `invalid_range` (`P0001`), checked against the caller's original `p_from`/`p_to` **before** the clamping below. All dates/ranges are Jerusalem-inclusive (`between p_from and p_to` on a `::date` already converted `at time zone 'Asia/Jerusalem'`).
...
- **Shared cars** (capacity denominator only — see "Retiring a car keeps its history" below for the historical-metrics rule, which differs): every shared car of the department not retired before the range started — `cars.type = 'shared' and (cars.retired_at is null or cars.retired_at::date >= p_from)` (Jerusalem-converted). **Superseded 2026-09-10** (`20260910100400_fix_department_stats_retired_cars.sql`): originally `cars.type = 'shared' and cars.status = 'active'`, the same predicate every ride-writing RPC uses to pick a usable shared car (`submit_request`, `try_auto_approve`, `place_series`, …) — that excluded a car retired *after* the range end from its own capacity, undercounting a range it was fully available for. A car mid-`maintenance` still counts (never excluded by either rule).
- **Active hours**: for every non-cancelled ride on a shared car of the department (any status — see "Retiring a car keeps its history" below) whose `[starts_at, ends_at)` could touch `[p_from, p_to]` at all, and for every Jerusalem calendar day in range that ride actually touches, the overlap of the ride with that day's `[06:00, 22:00)` Jerusalem window, in hours. Ordinary rides can only ever touch one calendar day in the current schema — `assert_same_day_window()` (`20260907102000_coordinator_planning_and_same_day_rides.sql`) rejects any ride whose start and end are not on the same Jerusalem day (`ride_must_end_same_day`) — but the query does not special-case that; it computes the overlap per touched day generically, so it stays correct if that constraint is ever relaxed for some ride kind.
...
- **policyScore**: the *latest* `siddur_versions` row per `week_start` (highest `version_no`) for every week whose **target week overlaps the requested range** (`week_start <= p_to and week_start + 6 >= p_from`; `week_start` is the Sunday, the week runs Sun–Sat), read for the department's currently-recorded active-policy score — `snapshot.policy_scores[]` entry whose `policy_version_id` matches the snapshot's own top-level `policy_version_id` (the "weighted coverage"/`alignment_ratio` field described in the publication note above), null or `0` ignored (a policy with no active version, or a publish that supplied an empty score snapshot, produces no countable week). `average` is the mean of the counted weeks' `alignment_ratio` (`0` when none); `weeks` is how many were counted. This is a *reporting* read only — it never calls `publish_siddur()` and does not participate in publication. **Bug fix (`20260914200000_department_stats_policy_score_by_week.sql`, 2026-09-14):** previously selected by `published_at` (Jerusalem date) instead of `week_start`, so a week published ahead of or after its own span — e.g. the Wednesday before it opens — silently fell outside a "last week" range query even though the target week itself was squarely inside it; selecting by `week_start` overlap matches every other per-week metric in this function (requests/rides/weekly).

**Extended metrics (`20260910098300_extend_department_stats_metrics.sql`, REQ item 78's 2026-09-10 owner follow-up)**:
- **requests.servedRate**: `granted / total` (`0` when `total = 0`), alongside the existing `unmetRate`.
- **distinctPeople** / **distinctDrivers**: over the same ride set as `rides`/`activeHours` (every non-cancelled ride on a shared car — any status, since 2026-09-10 — starting in range), `distinctPeople` is `count(distinct profile_id)` over the union of (a) `requester_id` of every request served by one of those rides (`ride_requests` join, `requests.status in ('assigned','merged')`), (b) `rides.driver_id` of those same rides (covers a chauffeur ride, which has no `driver` `ride_requests` row and so no served request of its own — the volunteer would otherwise be missed), (c) `request_companions.profile_id` of those same served requests, and (d) **`ride_passengers.person_id` of those same rides** (added 2026-09-14, `20260914190000_unified_ride_people.sql`, unified ride-people model — a directly-added passenger is a person on the ride exactly like a served request's requester/companion). A member with two served requests in range, who both drove and rode along, or who was simply added to a ride, counts once. Named children (`request_children`, and a `ride_passengers` row with `child_id` set) and `guest_passenger_names` are **not** profiles and are **not** counted. `distinctDrivers = count(distinct rides.driver_id)` over the same ride set, reported as a sub-line.
- **byRideType**: one entry per ride type touched by the same ride set — `{ rideTypeId, code, name, rides, hours }` — ride type taken from the ride's `driver` served request when one exists, else the lowest-`request_id` served request (covers chauffeur rides, deterministic tie-break); a ride with several served requests counts once, under that one ride type. `hours` reuses the exact `[06:00,22:00)` Jerusalem clipping `activeHours` uses, summed per ride type (a ride never spans two Jerusalem calendar days, so "per ride" and "per ride/day" coincide here). A ride with **no** served request at all (an administrative placeholder, or one the Sadran created but never assigned) falls into `{ rideTypeId: null, code: 'other', name: null }`. Ordered by `rides` desc, `rideTypeId` asc (nulls last) tie-break.
- **weekly**: one entry per week of the department whose `week_start` falls in `[p_from - 6, p_to]` (weeks *overlapping* the requested range, not just those fully inside it) — `{ weekStart, total, granted, unmet, cancelled, rides, provisional }`. A week currently `archived` with a cached `week_stats` row reads it verbatim (`provisional: false`); every other week (including `archived` weeks somehow missing a row, defensively) is computed **live**, over that week's own 7 Jerusalem days regardless of `p_from`/`p_to`, using the identical expressions `compute_week_stats()` uses (`provisional: true`). This is the "weekly graph of non-accepted requests" from REQ item 78: waiting-list placements settle *during* the week, so a live week's `unmet` is not yet final — only an archived week's cached figures are shown as such.

**`week_stats` (`20260910098200_create_week_stats.sql`, `capacity_hours` added `20260910100400`)**: `(department_id, week_start)` pk, composite FK to `weeks`; `total_requests`, `granted`, `unmet`, `cancelled`, `rides` int, `active_hours numeric`, `distinct_people` int, `capacity_hours numeric` (default `0`, `check (capacity_hours >= 0)`), `computed_at timestamptz`, `created_at`/`updated_at` (+ `set_updated_at()`) — not a "state table" (no `version`/`bump_version()`, nothing is ever concurrently edited by two users) and not audited (it is derived data, not a user action). RLS enabled + forced, one policy: `week_stats_select` — `is_admin() or is_sadran_any(department_id)`, mirroring `department_stats()`'s own authorization exactly. No INSERT/UPDATE/DELETE policy at all: `compute_week_stats(p_department_id uuid, p_week_start date) returns void` (`security definer`, revoked from `authenticated`/`anon`/`public`) is the only writer, upserting one row per call with the same definitions as `department_stats()`'s range-wide ones, scoped to `[week_start, week_start + 6]`. Called from two places: (1) `advance_week_phases()`'s `published`/`live` → `archived` transition (the tick runs every 15 minutes; this is the natural, idempotent moment to cache a week's final numbers, since waiting-list outcomes have settled by the time a week leaves `live` — DATA_MODEL step 17/ARCHITECTURE §10), and (2) the `20260910098200` migration's own one-time backfill, which computed it immediately for every week already `archived` before the migration ran (so existing archived weeks are not stuck without a cache until their next, nonexistent, phase transition). `capacity_hours` is not (yet) surfaced in `department_stats()`'s `weekly` array — it exists so an archived week's cache keeps its own historical capacity denominator instead of only ever reflecting today's fleet; exposing it is left for a future pass.

**Retiring a car keeps its history in the statistics (owner decision, 2026-09-10; `20260910100300_track_car_retired_at.sql`, `20260910100400_fix_department_stats_retired_cars.sql`)**. Cars are never deleted — admin removal only sets `status = 'retired'` — but `department_stats()` and `compute_week_stats()` had filtered inconsistently: `activeHours`/top-level `rides` (via the shared `candidate_rides` CTE) already used `cars.type = 'shared'` alone (any status), while `distinctPeople`/`distinctDrivers`, `byRideType`, the `weekly` series' per-week `rides`, and every metric inside `compute_week_stats()` (`rides`, `active_hours`, `distinct_people`) additionally required `cars.status = 'active'` — so retiring a car silently dropped its historical rides from some metrics but not others.

Rule now applied everywhere: every *historical* metric (`activeHours`, `rides`, `distinctPeople`/`distinctDrivers`, `byRideType`, `weekly`, and every `week_stats` column) counts rides on **any** shared car of the department regardless of current status — a retired car's past rides stay in the record forever, same as its own rows in `rides`/`requests`. Only the **capacity** denominator (`sharedCars`, `utilization.capacityHours`, `byWeekday.utilizationRate`, and the new `week_stats.capacity_hours`) is affected by retirement, via `cars.retired_at is null or cars.retired_at::date >= p_from` (Jerusalem-converted) — deliberately not `status <> 'retired'`, which would also exclude a car retired *after* the range, undercounting capacity for a range it was fully available for.

`cars.retired_at timestamptz` existed since `20260907090300_fleet.sql` but nothing had ever set it (admin removal is a direct `cars` table edit, not a dedicated RPC) — `20260910100300_track_car_retired_at.sql` adds the trigger `cars_track_retired_at()` (before insert or update) that fills a null `retired_at` on the transition into `status = 'retired'` and clears it again if the car is later un-retired, so a subsequent re-retirement gets a fresh timestamp; an explicit caller-supplied value (e.g. a backdated test fixture) is respected. There is no separate `status_changed_at` column.

**Correction to the brief that requested this fix**: it also asked to gate capacity on `created_at <= p_to` (a car "created after the range" should not count either). That reads right in isolation but breaks every far-past-week fixture in this codebase's own test convention — `supabase/tests/department_stats.sql`'s main fixture is built on `current_week_start() - 700` while the seeded cars' `created_at` is whenever `db reset` last ran, i.e. *after* `p_to`, not before it. `created_at` is a row-insertion timestamp, not a real fleet-acquisition date; the existing `earliest` clamp already keeps the reported range from reaching before the department's first real week/ride/request, which is the actual protection the brief was after. Dropped; reported as the correction.

Tests: `supabase/tests/department_stats.sql` case (k) — a dedicated department (catalogs copied from the main fixture's via `create_department(..., p_source_department_id)` so its ride/request rows have a valid destination/ride type), one active shared car and one shared car retired 30 days before the tested week, with one ride/request on the retired car inside the range: `sharedCars`/`utilization.capacityHours` count only the active car, while `activeHours`, `rides`, `distinctPeople`, `distinctDrivers` and `byRideType` all still include the retired car's ride; `compute_week_stats()` on the same fixture asserts `rides`/`active_hours`/`distinct_people` include it while `capacity_hours` excludes it.

**Statistics group S1-S4 (owner request 2026-09-14; `20260914110000_department_stats_sharing_indicators.sql`; docs/TODO.md "S — Statistics group"; REQ item 78 "Extended (2026-09-14)").** `department_stats()` redefined in place, full body copied forward, every existing field byte-for-byte unchanged in shape:

- **S1 — utilization counts the turnaround buffer.** The `hours_by_day` CTE's per-ride occupied end changes from bare `r.ends_at` to `least(r.ends_at + make_interval(mins => v_turnaround_minutes), that day's 22:00 window end)`, where `v_turnaround_minutes` is `department_settings.turnaround_minutes` for the department (default 30, defensively coalesced if the singleton row were ever missing) — **not** a per-week override, and **not** the per-ride `rides.turnaround`/`turnaround_override_minutes` actually applied at scheduling time; statistics deliberately use the department's plain configured default. This changes `utilization.activeHours`/`rate` and every `byWeekday[].avgActiveHours`/`utilizationRate` entry; `utilization.capacityHours` (`sharedCars × days × 16`) is a fixed denominator untouched by any one ride's span and is unaffected. `byRideType.hours` and `compute_week_stats()`'s own `active_hours` are deliberately **not** touched — out of this change's scope, and `compute_week_stats()`'s figure is never surfaced through `department_stats()`'s `weekly` array (which only exposes total/granted/unmet/cancelled/rides), so no inconsistency reaches the API.
- **S2 — `sharing` object** (owner: no combined score for now, three independent numbers): over the same "non-cancelled ride on a shared car, starting in range" set as `rides`/`activeHours`:
  - `peopleUtilization` = `sum(people) / sum(seats)` where, per ride, `people` = `sum(requests.adults + requests.child_seats + requests.boosters)` over its served (`ride_requests` → `requests.status in ('assigned','merged')`) requests, or `1` when the ride has a `driver_id` but no served request at all (the same convention `distinctPeople` already uses for a bare `rides.driver_id` — a chauffeur/administrative placeholder), **plus `count(*)` of that ride's `ride_passengers` rows** (added 2026-09-14, `20260914190000_unified_ride_people.sql` — each directly-added row is one more occupied seat regardless of its `seat_kind`, on top of whatever the served requests already contribute); `seats` = `max(car_seat_configs.adults + child_seats + boosters)` for the ride's car, falling back to `5` when a shared car has no seat configuration row at all. `0` when there are no rides in range (no division by zero).
  - `fragmentation` = `sum(n) / count(*)` over `(car_id, Jerusalem start day)` pairs with `n ≥ 1` non-cancelled ride — average rides per active car-day, always `≥ 1` by construction; raw values returned as `fragmentationRideCount` (the numerator) and `activeCarDays` (the denominator).
  - `oneWayFulfilment` = `count(status in ('assigned','merged')) / count(*)` over requests with `trip_shape <> 'round_trip'`, `status not in ('draft','withdrawn')`, `coalesce(depart_at, return_at)` (Jerusalem date) in range — the same day-key convention the top-level `requests` metric already uses, needed because a `one_way_from` request has a null `depart_at`. `0` when there are no such requests; raw values `oneWayServed`/`oneWayTotal`.
- **S3 — `cancellations` object**: over shared-car rides whose Jerusalem start date is in range and `status = 'cancelled'`, `total` = how many, `sameDay` = the subset where `(cancelled_at at time zone 'Asia/Jerusalem')::date = (starts_at at time zone 'Asia/Jerusalem')::date`, `sameDayRate = sameDay / total` (`0` when `total = 0`). The denominator is cancellations, not all rides (owner, docs/TODO.md Q7/A7: cancelling is fine, cancelling at the last minute is the problem).
- **S4 — `requestsByHour` array**: always 24 entries `{hour, count}` (`hour` 0..23), built from a `generate_series(0,23)` left-joined to a count of non-draft/non-withdrawn requests in range grouped by `extract(hour from (depart_at at time zone 'Asia/Jerusalem'))`. Filtered directly on `depart_at is not null` (not `coalesce(depart_at, return_at)`) — a `one_way_from` request has no departure to bucket by and is simply absent from every hour's count, not folded into hour `0`.

Tests: `supabase/tests/department_stats.sql` case (a) and (k) updated for S1's new numbers (ride A gains its 30-minute buffer: `2h → 2.5h`, so the main fixture's `activeHours` becomes `5.5` and Monday's `avgActiveHours` becomes `1.75`; the retired-car fixture's `activeHours` becomes `2.5`; `compute_week_stats()`'s own cached `active_hours` in that same fixture is deliberately left at `2.0`, since S1 does not touch it). New case (l), a dedicated department: two rides on one car-day plus one ride on a second car-day (`fragmentation = 1.5`, `fragmentationRideCount = 3`, `activeCarDays = 2`); one served ride (adults=2,child_seats=1 → 3 people), one driver-only ride with no served request (→ 1 person) and one served ride (adults=4,boosters=1 → 5 people) against a 5-seat car (`peopleUtilization = 9/15 = 0.6`); one ride cancelled the same day it started and one cancelled two days in advance (`cancellations.total = 2`, `sameDay = 1`, `sameDayRate = 0.5`); two served and one waitlisted one-way request plus a draft one-way request that must be excluded (`oneWayFulfilment = 2/3`); and `requestsByHour` pinned at hours 8/9/14 with hour 10 (the draft) and hour 17 (a `one_way_from` request with no `depart_at`) both asserted to stay `0`.
...
Tests: `supabase/tests/department_stats.sql` (registered in `scripts/test-db.mjs`) — a shared-car ride 05:00–08:00 (2h), one 21:00–23:30 (1h) and one 20:00–23:00 (2h) on a 14-day range so every weekday occurs exactly twice; a cancelled ride and a temporary-car ride are both ignored; requests spanning every counted/ignored status combination; two `siddur_versions` for one week (only the later version's score counts) and one for another week with a zero score (ignored, so `policyScore.weeks = 1`); a dedicated fixture proves the `week_start`-overlap selection itself — a week inside the range but published days *before* it (the owner's "Wednesday before" report) is still counted, and a week entirely outside the range whose siddur happens to have been published *inside* it is not; admin and the department's Sadran both read the same figures; a plain member is refused `not_authorized`; a Sadran of a *different* department (via `create_department`) is refused `not_authorized` for this one but reads their own (empty) department's stats fine, with a null `earliest`; `p_to < p_from` and a 402-day span are both refused `invalid_range`, a 400-day span is accepted. The main fixture's own week is now built on a *far-past* Sunday (`current_week_start() - 700`, not `+700`) precisely so its range is never touched by the `20260910098100` to-clamp — its `earliest` equals that week's own `week_start` (the smallest of its `weeks`/`rides`/`requests` dates, and far earlier than the seeded department's own live/open weeks). A separate small fixture (its own department, one `weeks` row, no cars/rides/requests) then isolates the clamp mechanics: `p_from` before `earliest` is raised to `earliest`; `p_to` after today is lowered to today; both together; and a department whose only data is a *future*-dated week (`earliest` after today) gets `days: 0` with the crossed `from`/`to` returned as-is rather than a negative day count or an error.

**Extended metrics cases (`20260910098300`)**: `servedRate` alongside `unmetRate`; a `distinctPeople`/`distinctDrivers` fixture — a request driving one ride, a second request riding along as passenger on the same ride, that same requester's *second* request driving a different ride (must still count once), a `request_companions` row on the first request (a profile with no request of its own), and a third ride left with no served request at all (its driver still counts via `rides.driver_id`, since chauffeur-style rides have no `driver` `ride_requests` row) — asserts the exact union count and the separate `distinctDrivers` count; `byRideType` — two ride types across the two served rides plus the unlinked ride's null-id `'other'` bucket, each with the expected `rides`/`hours`; `weekly` — the fixture's own (non-archived) week matches the aggregate requests/rides figures live (`provisional: true`), a second week is marked `archived` and its `week_stats` row is populated via a direct `compute_week_stats()` call, and `department_stats()` then reads that cached row verbatim (`provisional: false`); RLS on `week_stats` — the department's Sadran reads the cached row, a plain member reads zero rows; and a separate far-past week, `live`, whose window closed long ago, is archived by `advance_week_phases()` itself (not a direct `compute_week_stats()` call), asserting the phase flips to `archived` **and** a matching `week_stats` row now exists — exercising the cron hook, not just the function in isolation.

**Unified ride-people case (`20260914190000`, docs/tests §m)**: a dedicated department, one ride whose driver both drives and is the served requester of a 2-adult request (2 "people", 1 distinct profile via the existing driver/requester convention) plus one directly-added `ride_passengers` row (a different member, `seat_kind: 'adult'`, no request of its own) against a 4-seat car — asserts `distinctPeople = 2` (the requester **and** the added member, not just the one served by the request) and `sharing.peopleUtilization = round(3/4, 4)` (2 served + 1 added, over 4 seats).

### 7.7 Joinable rides before the waiting list — REQ §13.83 (F4, docs/TODO.md, owner 2026-09-14)

`joinable_rides_for_request(p_request_id uuid) returns table(ride_id, starts_at, ends_at, car_name, car_type, destination_name, driver_name, distance_km numeric, free_seats int, driver_phone text)` (`20260914130000_join_radius_and_joinable_rides.sql`, extended with `driver_phone` by `20260914160000_joinable_rides_driver_phone.sql`; `stable security definer set search_path = public, pg_temp`), granted to `authenticated` only, read-only (never called from `submit_request()`/`enter_waiting_list()` itself, so it can never affect placement). Authorization: the request's own `requester_id` or `can_manage_week(department_id, week_start)`, else `not_authorized`.

Returns no rows when: the request has no `destination_id` (a free-text destination — owner A8, no zone fallback); the request's week is not `published`/`live` (owner A10 — a defensive check, since a `waitlisted` `status` on the result only exists once `try_auto_approve()`/`enter_waiting_list()` has actually run, which itself only happens in those two phases). Otherwise, for every ride in the same department and week that is not cancelled and does not already serve this request:
- **Destination.** A ride's own `destination_id` column is car-location bookkeeping, not trip purpose (consistency decision 14 — a plain round-trip "keep" ride's `origin_id`/`destination_id` are both the department's home location, since the car ends the ride back home). The ride's actual destination is read from its **served request(s)** instead — the driver's own request when the ride has one, else any other served request (a chauffeur ride has no `driver`-role `ride_requests` row) — via `haversine_km(lat1,lng1,lat2,lng2) returns numeric` (great-circle distance, degrees in, km out; internal only, revoked from `public`/`anon`/`authenticated`, called only from inside this function's own `security definer` body). A ride whose resolved destination has no `lat`/`lng` (or whose distance exceeds `department_settings.join_radius_km`) is excluded.
- **Time.** The ride's `starts_at` (or, for a `one_way_from` request, `ends_at`) must be on the same Jerusalem calendar day as, and within ±120 minutes of, the request's own `depart_at` (or `return_at`).
- **Seats.** `free_seats` (returned, and required `≥ 1`) approximates the car's largest `car_seat_configs.adults` value minus the adults already served on the ride (the same "adults includes the driver" accounting `assert_ride_seats_fit()` uses, consistency decision 11) — a display estimate only; the real seat-fit invariant is still enforced by `submit_request()`'s own `assert_ride_seats_fit()` when the member actually asks to join.

Ordered by distance, then `starts_at`, then `id`. `driver_name` is `''` for a still-unclaimed ride. `driver_phone` (`profiles.phone`, added 2026-09-14) is `null` for the same reason, or when the driver has one but the request calling in is not otherwise entitled to see it under `phone_of()`'s own rule — this function reads `profiles.phone` directly rather than through `phone_of()`, since the owner amendment (REQ §10/§13.83, 2026-09-14: department phone numbers are not secrets) is specifically that this caller, who does not yet share the ride, should still see it; the ride detail's own `phone_of()`/`profile_phones()` gate (own/admin/Sadran/shares-a-ride) is unchanged everywhere else. The client uses `driver_phone` only for a WhatsApp quick-link button, alongside — not instead of — the existing "ask to join" flow (`join_ride_id`, §7.3 above).

`department_settings.join_radius_km numeric(5,1) not null default 10` (§4 table above) is the one new column; the admin settings screen (`src/features/admin/departments`) exposes it alongside the other weekly-cycle numbers.

Tests: `supabase/tests/joinable_rides.sql` — a within-radius ride with a free seat is returned; an otherwise-identical out-of-radius ride is excluded; the same ride on the next day is excluded; a full car (1-seat car, already carrying 1 adult) is excluded; a free-text (`destination_text`-only) sibling request returns no rows; another department member (neither the requester nor `can_manage_week`) is refused `not_authorized`; the Sadran (`can_manage_week`) can call it like the requester; the within-radius ride's `driver_phone` matches the seeded driver's `profiles.phone`.
...
| weeks, requests, request_companions, rides, ride_requests, siddur_versions, solver_runs (summary), week_stats | forever (stats, fairness lookback, §12 dashboards) | `solver_runs.summary` is small; published `siddur_versions.snapshot` includes per-policy, member and request scores for later comparison; `week_stats` is one small cached row per archived week, computed once (§7.6) and never recomputed |
...
- Publication carries complete per-policy board scores and a database fingerprint. `siddur_versions.snapshot.policy_scores` stores each applicable policy/version/name, request/served counts, total/served priority, weighted coverage and nested member/request/per-rule breakdowns. `profile_scores` retains the active-policy member breakdown. Freshness checks and snapshot persistence occur in the publication transaction. These are reporting data, not a publication precondition (`publishSiddur`/`publish_siddur()` accept an empty snapshot): `publishWithScores.ts` scores each policy **independently** — a scoring warning (`UNKNOWN_RULE_TYPE`, a rule-param error) is tolerated and logged rather than blanking the snapshot, and one policy throwing (bug fix 2026-09-14) only omits that policy's own entry, `console.error`-logged, leaving every other policy's score intact.
...
**Correction to this agent's brief (historical — superseded 2026-09-11)**: at the time this migration landed, `src/lib/enums.ts` did not yet exist, and neither did an `he.enums.*` namespace — `CLAUDE.md`'s "Enums are defined once, in SQL, mirrored into `src/lib/enums.ts`" convention had not been implemented for `notification_event`; `NotificationEvent` was used directly from the generated `Database['public']['Enums']['notification_event']` type. `src/lib/enums.ts` now exists (docs/REFACTOR_PLAN_2026-09-11.md E8) and exports `NOTIFICATION_EVENTS`/`NotificationEvent`/`notificationEventSchema`, checked against the generated type by `assertSameEnum`. The Hebrew mirror still lives at the top-level `he.notif` (snake_case keys, `satisfies Record<NotificationEvent, string>`) — there is still no `he.enums.*` namespace; labels for status-style enums live in `StatusBadge`'s per-kind maps instead.
...
- `src/lib/enums.ts` did not exist at the time (same correction as the 2026-09-09 car-care pass; it now does — see docs/REFACTOR_PLAN_2026-09-11.md E8): the new enum's Hebrew mirror went into the existing top-level `he.notif` dictionary, and the new `status_reason` codes into `STATUS_REASON_CODES` / `he.statusReason` in `src/i18n/he.ts`.
...
| `20260910096300_publish_siddur_days_weekday_letters.sql` | Patches `publish_siddur()`'s `days_agg` CTE (in-place via `pg_get_functiondef()`/`replace()`, same technique as `20260910096000`) to build `{{days}}` from `weekday_short_label(request_day)` instead of `to_char(request_day,'DD/MM')`, so it now renders e.g. `"א׳, ב׳, ו׳"` (owner decision). `outcomeLine`/`diffLine`'s per-line `{{day}}` was `DD/MM` at the time (since each line already carried a full date) — later changed to `day_date_label()` too, `20260915100000_day_date_label.sql`. |
...
## Department statistics: servedRate, distinctPeople, byRideType, weekly graph (2026-09-10, db-migrator)

Owner follow-up on REQ item 78 (§7.6 has the full field-by-field definition). Two migrations:

- `20260910098200_create_week_stats.sql`: new table `public.week_stats` (§7.6) caching, per `(department_id, week_start)`, the figures a week's own `department_stats()`-style range would report (`total_requests`, `granted`, `unmet`, `cancelled`, `rides`, `active_hours`, `distinct_people`); RLS enabled + forced, one `select` policy (`is_admin() or is_sadran_any(department_id)`), no write policy at all. `compute_week_stats(p_department_id, p_week_start)` (`security definer`, revoked from `authenticated`) computes and upserts one row, scoped to that week's own 7 days regardless of any caller-supplied range. `advance_week_phases()` is redefined in place (full body copied from the live `20260910095100_promote_upcoming_weeks_to_open.sql` definition): the existing `published`/`live` → `archived` `update ... where ...` is rewritten as a `for ... in update ... returning department_id, week_start loop` so each just-archived week's `compute_week_stats()` runs immediately in the same tick that archives it — the natural, idempotent moment, since waiting-list placements have settled by the time a week leaves `live`. The migration also backfills: every week already `archived` before this migration gets its `week_stats` row via a one-time `do $$ ... $$` loop, instead of waiting for a phase transition that, being already archived, will never happen again.
- `20260910098300_extend_department_stats_metrics.sql`: redefines `department_stats()` in place (full body copied from `20260910098100`'s live definition), adding `requests.servedRate`, `distinctPeople`/`distinctDrivers`, `byRideType` and `weekly` to the returned jsonb — every existing key is unchanged. §7.6 has the exact expressions; the short version: `distinctPeople`/`byRideType` are computed over the same "non-cancelled ride on a shared active car starting in range" set `rides`/`activeHours` already use, joined to `ride_requests`/`requests`/`request_companions`; `weekly` unions a `cached` CTE (reads `week_stats` for `archived` weeks in `[p_from - 6, p_to]`) with a `live` CTE (the identical per-week expressions, computed inline, for every other week in that range) and tags each with `provisional`.

**Design choices worth recording:**
- **`requester_id` vs `companion_ids`.** The brief's phrasing ("union `requests.requester_id` ... with the members in `requests.companion_ids`") predates the schema: `requests` has no `companion_ids` column. Companions are a join table, `request_companions(request_id, profile_id)` (`20260907090700_requests.sql`), already used exactly this way elsewhere (e.g. `weekly_sadran_permissions`'s `member_of`-style checks). `distinctPeople`/`compute_week_stats()` join it on the same served-request set. Reported as the requested correction.
- **`rides.driver_id` is part of the union, not just `requester_id`.** A chauffeur ride (REQ §13.65) has no `driver` `ride_requests` row — the volunteer drove without filing a request — so `requester_id` of a served request alone would miss them entirely. Adding `rides.driver_id` to the union (harmless for an ordinary ride, since its driver's own served request already has `requester_id = rides.driver_id`, DATA_MODEL §5.2's trigger) is what makes "as driver or passenger" true for every ride, not just requester-driven ones.
- **byRideType's "other" bucket is `rideTypeId: null`, not a real `ride_types` row**, even though the seeded catalog happens to have its own `code = 'other'` ride type (`00000000-0000-0000-0000-000000000025`, "אחר") — a ride whose driver genuinely picked that ride type is a distinct, real bucket with a non-null `rideTypeId`; the null-id bucket is specifically for a ride with **no served request at all** (an administrative placeholder). The test fixture deliberately avoids the seeded `'other'` ride type to keep the two cases visually distinct in assertions.
