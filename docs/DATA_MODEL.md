...
| ride_passengers | `member_of(department_id) ∧ (is_week_public(department_id, week_start) ∨ can_manage_week(department_id, week_start) ∨ person_id = auth.uid())` | **RPC only** — `set_ride_passengers()` (Sadran/admin or the ride's own driver) or `add_ride_passengers()` (any `member_of` once the week is public/managed); no direct policy | **RPC only** — same RPCs | **RPC only** — `set_ride_passengers()`/`remove_ride_passenger()` (the row's `added_by`, its `person_id`, the ride's driver, or a week manager) |
...
- **distinctPeople** / **distinctDrivers**: over the same ride set as `rides`/`activeHours` (every non-cancelled ride on a shared car — any status, since 2026-09-10 — starting in range), `distinctPeople` is `count(distinct profile_id)` over the union of (a) `requester_id` of every request served by one of those rides (`ride_requests` join, `requests.status in ('assigned','merged')`), (b) `rides.driver_id` of those same rides (covers a chauffeur ride, which has no `driver` `ride_requests` row and so no served request of its own — the volunteer would otherwise be missed), and (c) `request_companions.profile_id` of those same served requests. A member with two served requests in range, or who both drove and rode along, counts once. Named children (`request_children`) and `guest_passenger_names` are **not** profiles and are **not** counted. `distinctDrivers = count(distinct rides.driver_id)` over the same ride set, reported as a sub-line.
