...
- **`e2e/quick-request.spec.ts` fails every Friday after 10:00 Jerusalem time.** It clicks Friday 10:00 of the seeded *live* week (`FRIDAY_INDEX = 5`, `yForMinutes(600)`); once the wall clock passes that slot the quick-request form disables "קח/י את הרכב" with the past-slot tooltip, and the spec times out on the disabled button. Fix in the spec: pick the first slot at least two hours ahead of `now` on the current or next day, or freeze the browser clock (`page.clock`).
- **`e2e/board-coordination.spec.ts` (one-way drop → tight edit → merge proposal) fails on a fresh database and also fails on the pre-refactor commit `6a64257`** (same `toHaveURL(boardUrl)` after clicking propose; the retry then times out on the publish page). Not caused by the 2026-09-11 refactor. It is the longest single test in the suite and runs under the global 60 s budget; first suspect is time (add `test.slow()` or a per-test timeout and re-run), second is the proposal send never navigating back (check `ProposalComposerScreen` `navigate(returnTo)` after `sendMutation`). Owner/e2e-tester to triage.
