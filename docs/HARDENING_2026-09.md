...
- Edge functions' inline Hebrew error strings (hard rule 3) — done: `errorResponse()` (`supabase/functions/_shared/env.ts`) now returns `{error: {code}}` with no Hebrew; `eslint.config.js` bans Hebrew literals in `supabase/functions/**/*.ts` (`_shared/solver.js` excluded) so this cannot regress. Codes returned: `answer-proposal` — `rate_limited`, `missing_token`, `invalid_token`, `invalid_body`, `missing_answer_fields`, `proposal_expired`, `proposal_not_answerable`, `db_error`, `method_not_allowed` (mapped to Hebrew client-side in `src/features/proposals/api.ts`/`ProposalTokenPage.tsx`, reusing `he.memberErrors`/`he.proposalScreen` where a code is already surfaced); `push-dispatch`/`on-ride-cancelled` — `method_not_allowed`, `not_authorized`, `invalid_body`, `db_error`, `missing_offer_id`, `offer_not_found`, `missing_reference_data`, `no_active_policy` (no UI reader — pg_net/cron/admin-flush callers only, so these stay codes/English log lines, ARCHITECTURE.md §12).
