...
| A5 | ✅ done 2026-09-11 — `db:reset` clean, `db:types` no diff, 24/24 SQL suites pass; e2e 61 passed / 2 flaky / 3 failed, all three pre-existing or time-dependent (see docs/TODO.md "Flaky / time-dependent e2e specs"). Deployed the same day: `git push` (Cloudflare build), `supabase functions deploy` ×4, no migrations pending. **Schema never validated from a clean slate** since the 13 hardening migrations (`docs/TODO.md` "Waiting on…"). | M | owner runs | `npm run db:reset && npm run db:types && npm run db:test`, then `npm run test:e2e`. Commit regenerated types if they differ. Remove the TODO entry. |
...
| A8 | ✅ done 2026-09-11 — `.nvmrc` + `engines`. **No Node pin**: CI uses Node 20, no `.nvmrc`/`engines`. | S | any | Add `.nvmrc` = `20` and `"engines": {"node": "20.x"}`. | **Corrected same day**: first CI run failed — supabase-js 2.115 needs Node ≥22 (native WebSocket) and the old workflow/pin said 20; `.nvmrc` = 24 (matches the dev machine), `engines` = `>=22`.
...
| D4 | ✅ done 2026-09-11 (with E8) — both now import `TRIP_SHAPES`/`TripShape` from `src/lib/enums.ts`; also fixed `carStatusSchema`/`waitlistGroupStatusSchema` and the analogous `carCare/schema.ts` `CAR_ISSUE_CATEGORIES`/`TIRE_STATES` duplicates. `trip_shape` literal array defined twice: `requests/schema.ts:16` and `components/TripShapeControl.tsx:5`. | ui-dev | One export (in `lib/enums.ts` if Q1 = implement). |
...
| D13 | ✅ done 2026-09-11 — TS `tryAutoApprove` deleted (no caller); SOLVER.md §5.2 now documents the SQL-only rule set and its real test files (`hardening_semantics`, `notifications_semantics`, `rls_smoke`). Follow-up: same stale "form preview" claim in ARCHITECTURE.md:367/398, UX_FLOWS.md:255/1464, REQUIREMENTS.md:366 (docs pass). `solver/live.ts` `tryAutoApprove` has no caller; SQL `try_auto_approve` is the authority and nothing cross-checks them. | solver-dev + db-migrator | Either delete the TS copy and fix SOLVER.md §"auto-approve", or add one shared fixture asserted by both `live.test.ts` and `solve_semantics.sql`. |
| D14 | ✅ done 2026-09-11 — `bearerToken(req)` in `_shared/env.ts`; three sites (`push-dispatch`, `answer-proposal`, `destination-route/handler.ts`). `Bearer` header parsing duplicated in `push-dispatch` and `answer-proposal`. | db-migrator | `bearerToken(req)` in `_shared/env.ts`. | Follow-up same day: helper moved to Deno-free `_shared/request.ts` because `destination-route/handler.ts` is compiled by tsconfig.app.json and `env.ts` uses `Deno.env`.
...
| E8 | ✅ done 2026-09-11 — `src/lib/enums.ts` implemented (27 enums, each `as const satisfies readonly Enums<'x'>[]` + type + zod schema + `assertSameEnum<>()`), `enums.test.ts` (duplicate/parity checks). All ~30 ad hoc `Database["public"]["Enums"][…]` sites migrated except `src/features/sadran/board/components/BoardScreen.tsx` (no aliases found there — a different agent was mid-refactor on it; re-checked, nothing to migrate) and the deliberate independent-mirror tests (`unmetStatuses.test.ts`, `StatusBadge.test.tsx`), which now import the canonical arrays from `enums.ts` instead of hand-duplicating them. `he.enums` does not exist (labels stay in `StatusBadge`'s per-kind maps) — not created per the brief. `src/lib/enums.ts` absent; 30 ad hoc `Database["public"]["Enums"][…]` aliases (`RequestStatus` in 4 files, `WeekPhase` in 5, `RideStatus`/`ProposalStatus` in 4 each), 5 exhaustive maps in `StatusBadge`, 3 literal arrays. | L | ui-dev after Q1 | Implement as documented in CLAUDE.md Conventions (one `as const satisfies readonly Enums<'x'>[]` per enum + `assertSameEnum`), then replace aliases mechanically. |
