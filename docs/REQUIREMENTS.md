...
85. **Adding passengers to a published ride (owner, 2026-09-09/2026-09-14).** Any approved department member — not only the driver or a Sadran — may add named passengers (self, spouse/companions, children, or a free-text guest name) to any published ride, including a private/temporary car (posting a private car onto the siddur means being willing to share it). A "+ נוסעים" button on the siddur ride-detail sheet and the Sadran board's ride sheet opens a small picker (department members, department children, a free-text names field); rows land on the same `ride_passengers` table item 82 introduced, via a second RPC, `add_ride_passengers(ride_id, expected_version, passengers)` — unlike `set_ride_passengers()` (which *replaces* a manual reservation's whole people list and is Sadran/driver-only), this one only *appends*, is open to anyone once the ride's week is public (or the caller already manages the week), never touches `driver_id`, and silently skips a row that is already on the ride (as a `ride_passengers` person/child) or already served via a request/companion/child on that same ride, rather than erroring on it. Seat capacity is enforced exactly like `set_ride_passengers()` (`ride_seats_exceeded`). The ride's driver gets **one** notification per call naming who was added ("X הוסיף/ה את Y ו-Z לנסיעה שלך ביום א׳ לבנימינה"), unless the driver made the addition themself — the existing `outcome_changed` event, a new `passengers_added` variant (no new event: `notification_default_url()` already resolves a `ride_id` to the right siddur deep link, and it is not a Sadran-role event that bypasses a mute). A companion RPC, `remove_ride_passenger(ride_passenger_id, expected_version)`, removes one named row — allowed for whoever added it, the named person themself, the ride's driver, or a week manager; the UI shows a remove (×) next to a row only when the signed-in member is one of those four, though the RPC re-checks it regardless. The "+ נוסעים" button itself is disabled on a cancelled ride or before the week is public. (`add_ride_passengers`/`remove_ride_passenger` RPCs; DATA_MODEL §3.11 "Notification links…", §4.2/§4.3; UX_FLOWS §3.5, §4.2, §6.1)
