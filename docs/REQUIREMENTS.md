...
### 6.1 Car fields
Name, license plate, department, status (active / in maintenance / retired), notes (key location, quirks), features (roof rack, large trunk, automatic, 4x4…), **seat configurations** (§6.2), type (shared / temporary). Location is not a car field: it is derived from the rides of the day (§5.4). **Admins manage shared cars only** — the admin fleet screen has no type selector and always creates `type = shared`; temporary cars are member-owned (§6.4, item 53) — an admin can view and revoke one but never create one or change its type. (Owner decision 2026-09-10.)
...
- **Ask to join**: a member who sees a ride in the published siddur may ask to join it. This files a normal request carrying a hint to that ride (`join_ride_id`). On a **shared car** the Sadran sees it flagged and turns it into a merge proposal to the driver, with the usual consent. On a **temporary car** the merge proposal goes directly to the owner (the owner is the driver and decides); the Sadran sees it in the proposals list and is notified of the answer but is not in the loop (confirmed 2026-09-06). **Superseded for self-adding on an already published/live ride (owner, 2026-09-14, item 85):** once a ride's week is public, a member may instead add themself directly via the "+ נוסעים" flow (`add_ride_passengers`) with no proposal/consent round trip at all — this "ask to join" flow remains the only path before publication (an open/solving week), where a driver's consent still has to be sought.
...
- **A proposal cannot be created or sent for a day that is already published** (changed 2026-09-10): live coordination for a published/live day happens directly (WhatsApp/in person), not through the app — creating or sending one for such a day raises `proposal_day_public` instead of silently dying on the next tick. "Ask to join" (§7.3 above, `created_via = 'ask_to_join'`) is exempt from this guard on both ends, since it is filed by `submit_request()` itself, not composed by the Sadran, and still needs to reach the ride owner even on a published/live day.
...
- Members see the full published siddur of their department(s): who drives where and when (needed for merging and for finding a lift). They may also read the **published** siddurim of other departments, read-only, for lift-finding (confirmed 2026-09-06, §13.52). Phone numbers are visible only to Sadranim/Admins and to members sharing a ride, **except** the joinable-rides dialog's WhatsApp quick link (owner amendment, 2026-09-14, §13.83): a department's own phone numbers are not treated as secrets among its own members ("everybody knows everybody"), so a waitlisted member sees a `wa.me` link straight to the driver of a nearby ride they have not yet joined. This is a narrow, single-purpose exception — the ride detail's own phone-visibility rule (own / admin / Sadran / shares a ride, via `phone_of()`/`profile_phones()`) is otherwise unchanged everywhere else.
...
| QA process (owner decision, 2026-09-14) | Automated suites (unit ~15 s, SQL ~2 min, e2e ~6 min) are a full hard gate before every deploy (`npm run check:full`), but for day-to-day changes and manual QA effort the app is additionally split into impact **areas** — by coupling (a few shared pieces cut across feature folders: `RequestForm`, `WeekGrid`, `v_board_rides`, the solver, the notification pipeline), not by `src/features/*` folder — documented in `docs/TEST_MAP.md` and its machine-readable twin `test-map.json`. `npm run impact` diffs a change's files against that map and prints the affected areas, the exact Vitest/`db:test`/Playwright commands that already prove them, and a numbered manual QA checklist (exact Hebrew labels, seeded user, expected result) to paste into a PR or hand to a tester; a changed file matching no area is flagged so the map can be extended in the same change (CI runs `npm run impact -- --strict` and fails only on that gap). This narrows QA *effort*, it never replaces the automated gate. |
...
52. **Members may read other departments' published siddurim** (read-only, lift-finding). RLS: any approved member reads published rides, siddur versions and the requests they serve of any department (`is_week_public`); draft data stays department-scoped. (confirmed 2026-09-06; DATA_MODEL §4.3)
    **Amended 2026-09-14 (owner):** multiple departments are **not part of the MVP** — the UI's department switcher deliberately lists only the member's own departments, and browsing another department's siddur is not offered even though RLS would allow it. Future direction: a member of several departments both sees and requests in every department they belong to (one identity, several memberships); view-only browsing of foreign departments is not planned.
...
77. **Multi-day requests (2026-09-10).** A member may reserve a car from a departure day/time to a return day/time on a **later** day (a trip, a stay abroad, a week at the sea). Rules:
    ...
    - **Crossing into the next week.** A span may cross Saturday→Sunday. Legs that fall in a later week are materialized as **pinned** rides in that week as soon as the series gets a car, so the next week's Sadran sees the car as already taken and no solve run can move or delete them. **A leg reaching past the last week the department has opened no longer fails outright (updated 2026-09-10):** that week is materialized early, in a new `upcoming` phase — not open to an ordinary request, not visible to members, promoted to `open` automatically at its normal opening time — so a multi-day booking can still reserve its far end of the trip. The only remaining hard limit is a sane ceiling: a leg before the current week, or more than 6 weeks past the series' own first leg, is refused with `series_week_not_open` (MDR01). The member sees `he.errors.seriesWeekNotOpen` ("אפשר לשמור רכב עד שישה שבועות קדימה, ולא לימים שכבר עברו"). An admin manually opening a week that exists as `upcoming` promotes it to `open` (`open_week()`, 20260910095700).
    ...
    - **Only an explicit later return day makes a series (bug fix 2026-09-14).** The request form files a multi-day booking only when the member opened "חזרה ביום אחר?" and picked a later day; while that picker is closed the return day always follows the departure day (also backwards). Before the fix the form remembered a later default return day (the weekday of the member's previous request) and silently filed a half-hour request as a 7-day series, or failed with the same-day "עד 23:59" error.
...
78. **Department statistics (2026-09-10, owner request).** A statistics screen for the admin and the Sadran, backed by one RPC, `department_stats(department_id, from, to)` — an inclusive Jerusalem-calendar date range, capped at 400 days, over five metrics:
    ...
    - **Policy score.** The average of the department's recorded weighted-coverage publication score (DATA_MODEL §3.9) across the weeks **whose target week overlaps the range** (`week_start`, not `published_at` — bug fix 2026-09-14: a week published ahead of or after its own span, e.g. the Wednesday before it opens, must still count when that week itself is in range), ignoring weeks with no score.
    Only the admin or a Sadran of the department may call it. The response also reports the department's `earliest` date with any data and silently clamps the requested range to `[earliest, today]` (Jerusalem) rather than showing capacity for days before the department existed or after today. (DATA_MODEL §7.6)
    **Extended (2026-09-10, owner follow-up)** with four more metrics, all framed positively (the UI leads with what was *served*, not what was refused):
    - **Served rate.** `requests.servedRate = granted / total` (`0` when `total = 0`), alongside the existing `unmetRate`.
    - **Distinct people.** How many distinct member profiles actually rode in a shared car in range, as driver or passenger — not requests, not seats. A member with several requests in range, or who both drove once and rode along once, counts once. Named children and unnamed guest passengers are not profiles and are not counted. A `distinctDrivers` sub-line reports how many distinct people drove.
    - **By ride type.** The same served-ride set broken down by ride type (עבודה/ילדים/בריאות/סידורים/אחר, plus an "other" bucket for a ride that ended up carrying no named request at all), each with a ride count and active hours — so a Sadran can see "most of our driving is childcare" alongside the existing by-weekday view.
    ...
    **Retiring a car keeps its history in the statistics** — every historical metric above (active hours, rides, distinct people/drivers, by ride type, the weekly graph) still counts a retired shared car's past rides; only capacity (`capacityHours`/`sharedCars`/`byWeekday.utilizationRate`) counts non-retired shared cars. (2026-09-10 owner follow-up; DATA_MODEL §7.6)
    (DATA_MODEL §7.6, `week_stats`)

    **Extended (2026-09-14, owner request)** with four more figures (docs/TODO.md "S — Statistics group"; `20260914110000_department_stats_sharing_indicators.sql`):
    - **S1 — Utilization counts the turnaround buffer.** Every ride's occupied span used to compute `utilization.activeHours`/`rate` and `byWeekday.avgActiveHours`/`utilizationRate` is now `[starts_at, least(ends_at + department_settings.turnaround_minutes, that Jerusalem day's 22:00 window end)]` instead of bare `[starts_at, ends_at]` — the turnaround buffer after a ride still ties up the car, so it counts as "active" time too. `capacityHours` (shared cars × days × 16) is unchanged, a fixed denominator unrelated to any one ride; only the numerator grows. Rationale (owner): one 8-hour ride and one 3h + one 4h ride on the same car should score about the same, since the gap between the two shorter rides still occupies the car.
    - **S2 — Same-day sharing indicators**, a new `sharing` object — three independent numbers, deliberately **no combined score** (owner: not yet). Over the same "non-cancelled ride on a shared car, starting in range" set `rides`/`activeHours` use: **`peopleUtilization`** = (Σ people on board per ride) ÷ (Σ seats available per ride) — people on board is the adults+child_seats+boosters of the ride's served requests, or 1 when the ride has a driver but no served request at all (an administrative/chauffeur placeholder); seats available is the car's largest seat configuration (adults+child_seats+boosters), falling back to 5. **`fragmentation`** = (Σ rides, over every (shared car, Jerusalem start day) pair with ≥ 1 ride) ÷ (number of such pairs) — the average rides per active car-day, always ≥ 1 by construction (raw values returned as `fragmentationRideCount`/`activeCarDays`). **`oneWayFulfilment`** = one-way requests (`trip_shape ≠ round_trip`, non-draft/non-withdrawn, departing in range) that ended up assigned/merged, divided by all such requests (raw values `oneWayServed`/`oneWayTotal`).
    - **S3 — Same-day cancellation rate**, a new `cancellations` object: over shared-car rides that started in range and were cancelled, `total` = how many, `sameDay` = the subset whose `cancelled_at` falls on the same Jerusalem calendar day as `starts_at`, `sameDayRate = sameDay / total` (0 when there were no cancellations at all). The denominator is cancellations, not all rides — cancelling is fine; cancelling at the last minute is the problem, and the UI caption stays neutral.
    - **S4 — Requests per hour of day**, a new `requestsByHour` array — always 24 entries `{hour, count}` (0..23, Asia/Jerusalem), counting non-draft/non-withdrawn requests by the hour of their `depart_at` (not `coalesce(depart_at, return_at)`: a `one_way_from` request has no `depart_at` at all and simply has nothing to bucket by, so it is not counted here).
    (DATA_MODEL §7.6)
...
80. **A private car appears in the siddur only on days it has a ride (owner, 2026-09-14).** In the day grid, a temporary car's row is shown only when that car has at least one ride on the displayed day; on other days it is simply absent (a private car that is "not going anywhere" is not information members need, and its empty row pushes the shared cars around). Shared cars are always listed, rides or not. Applies to the member siddur and, since the owner's same-day follow-up (2026-09-14), to the Sadran board grid as well; the board's phantom lanes are unaffected. (UX_FLOWS §3.5, §4.2)

81. **Per-week request closing time and the three-state new-request button (owner, 2026-09-14).** Two related changes (docs/TODO.md "F2"):
    - **A Sadran may change one week's request-closing time**, shortening or extending it (owner A2/A4), from the board kebab menu ("שינוי מועד סגירת הבקשות") via `set_week_close_at(department_id, week_start, close_at)`: allowed only for `can_manage_week()` while the week's phase is `open` or `solving` (`week_close_not_editable` otherwise); `close_at` must land strictly after `open_at` and no later than `publish_at`, on the 15-minute grid (`week_close_out_of_range`). Setting a `close_at` already in the past while `open` flips the phase to `solving` immediately and sends the Sadranim the same `window_closed_solve_now` notice `advance_week_phases()` would send once the deadline elapsed on its own; setting a future `close_at` while `solving` reopens the week to `open` (the only place this reverse transition exists). `advance_week_phases()` needs no change — it already reads `weeks.close_at` fresh on every tick, so a Sadran-shortened or -extended deadline is respected automatically.
    - **Members are notified whenever the window changes** (owner A2): a new `window_changed` notification event (25th canonical event, UX_FLOWS §6.1), sent to every approved member of the department with the new closing weekday/time interpolated ("הבקשות לשבוע {{weekLabel}} נסגרות ביום {{closeDay}} בשעה {{closeTime}}"); mutable (members may mute it, grouped with `window_open`/`window_closing` in the profile's mute preferences).
    - **The "בקשה חדשה" button becomes three/four-state** instead of an always-on link to `/requests/new` (`newRequestButtonState()`, pure, reused by the floating "+" and any inline "בקשה חדשה" action so there is exactly one place this logic lives): let the **target week** be the newest non-archived week that is not `live` (the next week), falling back to the `live` week when nothing newer exists yet (a candidate whose phase is `upcoming` — materialized early only for a multi-day series leg, §13.77 — doesn't count as "next" either, so this falls back the same way).
      1. Target phase `open` → **"בקשה חדשה"** (owner revision 2026-09-14: plain label; the button still refers to next week), links to the request form for that week.
      2. Target phase `solving` → disabled, **"סידור בהכנה..."**.
      3. Target phase `published` → **"רשימת המתנה לשבוע הבא"**, links to the request form for that week (already auto-approves/waitlists correctly on a published week per item 24/§13.66).
      4. **Edge case (owner-confirmed, 2026-09-14; revised same day — the button must always read as being about *next* week, never "this week"): only the `live` week exists and nothing newer is open yet** → **disabled**, same label as state 1, **"בקשה חדשה"** — nothing to link to yet, same muted/`aria-disabled` treatment as state 2.
      Live-week quick requests (the board's empty-slot "+" and "car now") are a separate, unaffected entry point (owner A3).
    (DATA_MODEL §2, §3.11, §6; UX_FLOWS §4.2, §6.1; relevant screen sections)

82. **Reservations can name people (owner, 2026-09-14).** A Sadran-created manual reservation ("שמירת זמן") can now optionally name department members (the first picked becomes the driver) and children on it via a `ride_passengers` table and `set_ride_passengers()` RPC. Named people see the ride in their "my rides", are notified when added via the `outcome_changed` event with a new `reservation_added` variant, and are notified on full ride cancellation. The same table/RPC is the reusable shared base for the not-yet-built "+ נוסעים" feature (adding named passengers to any published ride by a button, priority 3 backlog item).

83. **Joinable rides offered before the waiting list (owner, 2026-09-14).** Right after a member's request lands on the waiting list — a plain waiting-list submission or an explicit "enter the waiting list" against an already-**published**/**live** week, never an open/solving-week submission — the client offers existing rides instead of just leaving the member to wait: "נכנסת לרשימת ההמתנה — אבל יש אפשרות נוספת" ("you are on the waiting list, HOWEVER here is another option instead of just waiting"). A ride qualifies when, that same Jerusalem day, in the same department, it: is not cancelled and is not the request's own ride; goes somewhere within `department_settings.join_radius_km` (default 10 km, admin-editable) of the request's own **preset** destination (a free-text destination, on either side, is skipped entirely — no fallback to zone); starts within ±120 minutes of the request's `depart_at` (or, for a `one_way_from` request, ends within ±120 minutes of its `return_at`); and has at least one free seat. Offered rides are ordered by distance, then departure time. **In-app action, updated for the unified ride-people model (owner, 2026-09-14, item 85):** the one-tap button per offered ride, "הצטרפות לנסיעה", now joins that ride directly — it calls `add_ride_passengers()` with the just-filed request's own people (the requester as an adult, plus its companions/children/guest names) and then withdraws the now-redundant waitlisted request (`withdraw_request()`), instead of the earlier "ask to join" flow (opening a second `/requests/new?ride=<id>` request for the driver to answer). A failure (e.g. `ride_seats_exceeded`) surfaces its error toast and leaves the waitlisted request untouched. **Owner amendment (2026-09-14, §10):** alongside it, a WhatsApp icon button opens a prefilled `wa.me` chat straight to the ride's driver — department phone numbers are not treated as secrets, so `joinable_rides_for_request()` also returns `driver_phone` (`profiles.phone`, read directly rather than through `phone_of()`'s own-department/shares-ride gate, since the whole point here is a member who does not yet share this ride); the button is hidden when the driver has no phone on file or the ride has no driver yet. A second button, "להישאר ברשימת ההמתנה", continues the ordinary waiting-list outcome unchanged. (`joinable_rides_for_request(request_id)`, `add_ride_passengers()`, `withdraw_request()` RPCs; DATA_MODEL §3.1, §7; UX_FLOWS §3.4/§18/§24, §5.1)

84. **Mileage balancing across shared cars, and a `carChoice` policy option to rank it against packing (owner, 2026-09-14).** When the solver's car choice for a request is otherwise indifferent — every other consideration (preferred car, seat fit/seat configuration, features like trunk size, car availability/chain, the member's usual car) has already tied between two or more cars — it prefers the shared car with the lower cumulative distance: its own rolling **4-week** mileage total (owner A11: a rolling window, not "this week only"; no department setting) plus whatever this same solve has already put on it (so a second same-destination request later in the week can land on a different, less-driven car than the first). Distance is the request's destination `distance_km` (0 for unknown/free-text), counted twice for a round trip and once per leg for a relay pair. This **never overrides** preferred car, seat fit, availability or the member's usual car — it only decides among cars that were already equally acceptable — and it **never forces a manual move**: the Sadran sees it only as a visible reason on the assignment (owner A12), with no confirmation prompt when a car is changed by hand afterward.

    Where mileage balance ranks against the solver's internal best-fit packing heuristic (across days, best-fit alone prefers the car whose remaining free window is already shorter — the car that drove yesterday — which is exactly the piling-up mileage balance exists to undo) is an admin-editable, per-policy-version option, `carChoice: 'pack' | 'spread'` (owner decision 2026-09-14, superseding the earlier "lead decision, for the owner to confirm"):
    - **`spread`** (default; absent on every version predating this option = `spread`, so existing policies are unaffected): mileage ranks above packing — the piling-up packing alone would cause is undone, spreading mileage more evenly across the fleet.
    - **`pack`**: packing ranks above mileage — rides pack onto already-used cars, keeping whole cars genuinely free ("more available resources" for other uses) rather than lightly touching every car. Mileage still breaks a genuine tie once packing itself has tied.

    Edited on the priority-policy editor (§7.2, `/admin/policies/:id`, UX_FLOWS §5.8) alongside the rule weights, saved with the version (immutable history, like the rules themselves), and takes effect once that version is active/selected — never a global or department-level toggle. (`car_mileage_totals(department_id, week_start, weeks default 4)` RPC; `policy_versions.settings` jsonb; `create_policy_version(..., p_settings)` RPC; `src/solver` `Car.mileageKm`, `Policy.carChoice`, `reasonCode: 'CAR_BALANCED_MILEAGE'`; SOLVER.md §3.6/§3.6.2)

85. **One passenger list per ride; anyone adds or removes (owner, 2026-09-14).** A ride has ONE passenger list, not two kinds of people shown differently: someone filed through a request (the requester, `request_companions`, `request_children`, or a request's own free-text guest names) and someone added directly (`ride_passengers`, item 82's manual-reservation naming plus the "+ נוסעים" button below) are the same kind of thing — shown, counted and edited identically. `v_board_rides.people` is the single unified list (one entry per person, driver first then by name; DATA_MODEL "`v_board_rides`" `people` contract), each entry carrying a stable `key` (`driver:<profile_id>`, `req:<request_id>` for a non-driver requester, `comp:<request_id>:<profile_id>`, `child:<request_id>:<child_id>`, `guest:<request_id>:<n>`, or `added:<ride_passenger_id>`) that both the "+ נוסעים" add flow and its removal counterpart address. **Seat accounting (2026-09-15):** a named person is listed by name only, never marked as a child; the "מבוגר/ת 1" / "ילד/ה 1" placeholders exist solely for seats nobody is named for. A named child counts once — the request form and `set_request_children()` share one seat contract (UX_FLOWS §4.2 bugfix 2026-09-15).

    **Anyone may add or remove anyone.** Any approved department member — not only the driver or a Sadran — may add named passengers (self, spouse/companions, children, or a free-text guest name) to, or remove any one person from, any ride of a **published or live** week (never a cancelled ride, never an unpublished week; a Sadran/admin may still act on an **open/solving** week too, same `can_manage_week` escape hatch every other RPC in this area has). This includes a private/temporary car (posting a private car onto the siddur means being willing to share it). The **driver can never be removed this way** — cancelling the ride is the path for that. Self-adding this way replaces the in-app "ask to join" flow on an already published/live ride (§7.3, decisions 12/19 still govern proposal-based joining before publication); members are trusted to have coordinated off-app, which is why notifications matter here.

    A "+ נוסעים" button on the siddur ride-detail sheet and the Sadran board's ride sheet opens a small picker (department members, department children, a free-text names field); rows land on the `ride_passengers` table item 82 introduced, via `add_ride_passengers(ride_id, expected_version, passengers)` — this *appends* (never replaces, unlike `set_ride_passengers()`, still the Sadran/driver-only *replace* editor for a manual reservation's whole list) and silently skips a row already on the ride or already served via a request/companion/child on that same ride, rather than erroring on it. Seat capacity is enforced exactly like `set_ride_passengers()` (`ride_seats_exceeded`). The companion removal RPC is `remove_ride_person(ride_id, expected_version, key)`, keyed by a `people[].key` from above: an `added:` key deletes the `ride_passengers` row; `comp:`/`child:`/`guest:` edit the underlying request (drop the companion/child/guest name, bumping that request's version); `req:` withdraws that (non-driver) request's participation in this ride — the ride and its other passengers are unaffected, the request itself ends `withdrawn`; `driver:` is always refused (`ride_driver_not_removable`).

    Notifications (`outcome_changed` variants): **on add** — the driver gets one aggregate notice per call naming everyone added (existing `passengers_added`, unless the driver made the addition themself), and each newly-added *member* additionally gets their own notice (`passenger_added_you`, unless they added themself). **On remove** — the driver is told who was removed (`passengers_removed`, unless the driver removed them), and the removed member is told too (`passenger_removed_you`, unless they removed themself); a removed **child**'s parent(s) (`children`/`child_guardians`) are told instead (`child_removed`), unless a parent did the removing. A free-text guest (no `person_id`/`child_id`) triggers **no notification to anyone**, in either direction beyond the driver's existing add-time aggregate notice. No new notification event: `notification_default_url()` already resolves a `ride_id` to the right siddur deep link, and none of this is a Sadran-role event that bypasses a mute. The "+ נוסעים" button itself, and every remove (×) affordance, is disabled/hidden on a cancelled ride or before the week is public — the RPCs re-check regardless. (`add_ride_passengers`/`remove_ride_person` RPCs; DATA_MODEL §3.11 "Notification links…", §4.2/§4.3, "`v_board_rides`" `people` contract; UX_FLOWS §3.5, §4.2, §6.1)

86. **Every single date shown to members carries its weekday, format ד׳ 16.9 (owner 2026-09-14/15).** (`day_date_label()`, `20260915100000_day_date_label.sql`; DATA_MODEL §3.11; UX_FLOWS §6, `formatDayDate`/`src/lib/dayLabels.ts` on the TS side.)
...
- Publishing recalculates the final board score against every applicable policy profile (all current profiles belonging to the department, including inactive ones), as clarified by the owner. Persist policy version, served/total priority, weighted coverage and per-request rule breakdowns with the publication for later review of manual changes against alternative policies.
  - The same weighted coverage figure also shows **live** on the Sadran board's policy chip (owner, 2026-09-14), next to and per row inside its policy-versions dialog — recomputed client-side from the board's current assignments whenever the selected policy or the board changes; purely a preview, nothing is stored until publish.
