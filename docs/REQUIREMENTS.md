...
80. **A private car appears in the siddur only on days it has a ride (owner, 2026-09-14).** In the day grid, a temporary car's row is shown only when that car has at least one ride on the displayed day; on other days it is simply absent (a private car that is "not going anywhere" is not information members need, and its empty row pushes the shared cars around). Shared cars are always listed, rides or not. Applies to the member siddur only — the Sadran board keeps every row. (UX_FLOWS §3.5, §4.2)
