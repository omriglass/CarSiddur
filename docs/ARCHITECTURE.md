...
| §13.78 department statistics: only the admin or the department's Sadran reads it | statistics screen (admin + Sadran) | – | `department_stats(dept, from, to)` (`SECURITY DEFINER stable`, `is_admin() or is_sadran_any(dept)` else `not_authorized`); `invalid_range` for `to < from` or a span over 400 days; no table, no RLS change (`DATA_MODEL.md` §7.6) |
