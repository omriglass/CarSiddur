...
- **Edge Functions** return `{error: {code}}` (a stable machine code, never Hebrew — CLAUDE.md hard rule 3) with proper HTTP codes; callers with a UI reader (`answer-proposal`, `destination-route`) map the code to Hebrew client-side (`src/features/proposals/api.ts`, `src/features/admin/destinations/api.ts`), reusing `he.memberErrors`/`he.adminDestinations` where the code already has an entry. `push-dispatch`/`on-ride-cancelled` have no UI reader (called only by pg_net/cron or an admin flush) so their codes/log lines stay plain English. The client never shows raw errors. Push failures never fail the business transaction (outbox pattern).
