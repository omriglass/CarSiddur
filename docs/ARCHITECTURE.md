...
carshare-nevo/
├─ src/
│  ├─ app/                  Router, providers (Query, Auth, Theme, RTL dir), layout shell, route guards
...
│  ├─ lib/                  Pure helpers: time (Asia/Jerusalem), week math, push, deep-link, whatsapp url
...
The residual risk — a forwarded message lets someone else answer — is accepted for a kibbutz-scale, low-stakes decision that the Sadran sees and can reverse. If the browser *does* have a session, the page shows the full UI (inbox link, my-week) in addition to the answer buttons.
