...
- **Live phase** (`live.ts`): `matchFreedSlot()` filters series requests out of its candidate pool; `tryAutoApprove()` returns `null` for any request with `seriesId` set — SQL's `try_auto_approve_series` (owned by the DB layer) handles that case, since all-or-nothing placement across every leg's day needs the whole-series view these single-slot helpers deliberately don't have.
