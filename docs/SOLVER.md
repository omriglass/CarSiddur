...
5. One-way legs without a host or a partner (REQUIREMENTS §7.1 item 5), in this order:
   ...
   - `chauffeur` — for any one-way leg (`passenger` without host, `relay` without partner): offered only when it is actually possible (owner, 2026-09-14; gated in `suggestions.ts` via `chauffeurLoad()` from §3.3) — some shared car is free at `H` for the whole §1.2 chauffeur window **and** its seat configuration fits `chauffeurLoad(nr.passengers)` (the requester's load plus one adult for the volunteer); the first qualifying car in the existing deterministic car order is used, otherwise no `chauffeur` suggestion is emitted for that request. When one qualifies, the request is shown as **needs a driver** (`stats.needsDriver`), the Sadran assigns a volunteer; `confidence = 0.5`. `volunteerCandidateMemberIds` lists members already driving that day with no overlapping ride, sorted by id.
