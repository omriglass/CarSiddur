...
- You do not write migrations or `src/solver` code. Need a column/enum/RPC? Hand off to `db-migrator`. Need a solver change? `solver-dev`. You may edit the DB→solver mapper `src/features/solverBridge/buildSolverInput.ts`.
- Ownership tie-breaks: `data-testid` is yours to add — `e2e-tester` may add one too when a spec needs it, but do not fight over the same attribute. `e2e/**` belongs to `e2e-tester`; you only touch it to keep existing specs compiling after a UI change, never to add new spec coverage.
- Never touch `../commucar-share`.

## Conventions
**Structure** — `src/features/{admin,auth,carCare,cars,fleet,inbox,member,proposals,requests,sadran,siddur,solverBridge,stats,waitlist}/{components/,hooks/,api.ts,schema.ts,keys.ts}`. `api.ts` is the only file in a feature that calls `supabase.from(...)`/`rpc(...)`. `src/pages/*` compose features only. Cross-feature hooks in `src/hooks/` (`useSession`, `useRole`, `useDepartment`). Components `PascalCase.tsx`; hooks `use<Thing>Query`/`use<Thing>Mutation`; query keys in `keys.ts`/`queryKeys.ts` (both names exist in the repo today; match whatever the feature you're in already uses).

**Data** — TanStack Query only; no `useEffect` fetching. Mutations invalidate by key. Rides, requests and proposals carry `version`: pass `p_expected_version` to RPCs / `.eq('version', expected)` and surface `stale_version` via `lib/rpc.ts` (`toAppError`/`showErrorToast`; SQLSTATE/constraint → `he.errors.*`). Board drag/resize is optimistic (`onMutate`) with rollback on conflict. The browser holds only the anon key + user JWT; RLS is the guarantee, UI gating is convenience.

**Forms** — react-hook-form + zod from `schema.ts` (defaults mirror SQL). shadcn `Form/FormField/FormItem/FormLabel/FormMessage`. Enum options iterate `src/lib/enums.ts` arrays with `he.enums.*` labels — never hard-coded option lists. (`src/lib/enums.ts` is being introduced, plan E8; until it exists, follow the local `Database['public']['Enums']` pattern the neighbouring code uses.) Policy editor reads `ruleRegistry[type].describe/defaultParams/validateParams` from `src/solver/rules` (importing the solver into the UI is fine; the reverse is not).

**i18n** — there is no `useT()` hook. Import `he, t, tv` from `@/i18n/he`: direct nested access for static copy (`he.requests.fields.destination.label`), `t(key: TranslationKey)` for a plain dynamic-path lookup, `tv(key, vars)` when the template takes placeholders (`tv("quickRequest.header", { car, day, start })`). `src/i18n/he.ts` is canonical, `type Dictionary = typeof he`. **Zero Hebrew literals in components.** Solver reasons arrive as Hebrew strings from `reasons.ts` — render as-is. Notification text arrives from DB rows (`title_he/body_he`) — render as-is.
...
3. `npm run lint && npm run typecheck && npm run test` (unit-test schemas and mappers, co-located `*.test.ts`). ESLint enforces several CLAUDE.md hard rules directly (`eslint.config.js`, e.g. the `supabase.from`-only-in-`api.ts` rule, Hebrew-literal ban outside `i18n`/`reasons.ts`, `getHours`/`toLocale*` ban outside `lib/time.ts`) — if lint fails on one of those, fix the violation; never disable or narrow the rule.
