...
- Notifications: emit via `enqueue_notification(recipient, event, dept, week_start, vars, data, dedupe_key)` (check the actual signature in `20260907091200_notifications.sql`); it writes `notifications` + `push_outbox`; events are the 18 of UX_FLOWS §6.1; copy lives in `notification_templates` seed rows, never Hebrew in function bodies.
...
4. Mirror enums in `src/lib/enums.ts` (const + zod + `assertSameEnum`); `npm run typecheck`.
5. Tests: `supabase test db` (`supabase/tests/rls_spec.sql` — every table RLS'd, no `true` write qual) plus a table-specific two-users/two-departments case and one RPC refusal case.
...
8. Report: migration files, enum changes, tables/policies/RPCs, seed changes, doc sections, hand-offs (labels, mappers, UI).
