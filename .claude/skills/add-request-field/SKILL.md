...
- [ ] Run `npm run impact`; update `docs/TEST_MAP.md`/`test-map.json` (usually the **request-form** area, and **quick-request**/**waitlist** too if the field reaches `RequestForm.tsx`'s shared paths) in the same change, then paste the printed QA checklist into the PR/report.
