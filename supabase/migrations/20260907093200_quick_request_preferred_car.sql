-- Product feature (2026-09-07): "quick request from an empty slot" — clicking an empty
-- cell on the published (live-week) siddur grid should let a member say "I'm taking THIS
-- car" in one short sheet. REQUIREMENTS §8 "new request on a free car" already auto-approves
-- a live-week round trip onto *any* free shared car; this adds an optional car preference the
-- member can express, tried first, with the existing behavior kept as the fallback.
--
-- DATA_MODEL.md §6.1 item 24.

alter table public.requests add column preferred_car_id uuid references public.cars(id) on delete set null;

comment on column public.requests.preferred_car_id is
  'Optional car the member asked for when filing (quick-request-from-empty-slot flow, UX_FLOWS.md §18). try_auto_approve() tries it first in live weeks, then falls back to any free car exactly as before; visible to the Sadran alongside the request, never required.';

-- ---------------------------------------------------------------------------
-- try_auto_approve: return type changes (request_status -> jsonb, so the caller learns
-- *which* car — or that none was free), so this needs drop + create, not create or replace.
-- Body otherwise reproduced from 20260907092800_fix_requests_status_guard_system_transitions.sql
-- (the trusted-transition flag), with the preferred-car branch added.
-- ---------------------------------------------------------------------------
drop function public.try_auto_approve(uuid);

create function public.try_auto_approve(p_request_id uuid) returns jsonb
...
-- ---------------------------------------------------------------------------
-- submit_request: accepts `preferred_car_id`, stores it (coalesced on the update branch —
-- same optional-field convention as adults/child_seats/…, so re-saving through the plain
-- edit form, which never sends this key, does not silently wipe it), and merges
-- try_auto_approve()'s richer jsonb into its own response — additive keys only
-- (`status`/`ride_id`/`car_id`/`reason`), so every existing caller reading
-- request_id/is_late/warnings is unaffected. Body otherwise reproduced verbatim from
-- 20260907092800_fix_requests_status_guard_system_transitions.sql.
-- ---------------------------------------------------------------------------
