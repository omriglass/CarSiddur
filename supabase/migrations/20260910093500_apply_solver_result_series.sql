-- REQ §13.77 — apply_solver_result() drags a multi-day series along.
--
-- Regenerated in full from the live definition (last changed by
-- 20260907093300_apply_solver_result_atomic_summary.sql) with three additions:
--   1. each inserted ride carries `series_id`, copied from the served request, at INSERT
--      time — `rides_before_write()` needs it before any ride_requests row exists;
--   2. after the rides/statuses are written, every series touched by this apply is handed
--      to `place_series()` on the car the solver chose, which fills in the legs the solver
--      never saw (other weeks) as pinned 'SERIES_CARRY_OVER' rides;
--   3. a series the car cannot take for the whole span is rolled back on its own (its rows
--      from this apply are deleted, its legs return to `submitted`/'SERIES_CAR_UNAVAILABLE')
--      and reported in the new summary key `skippedSeries: [{series_id, reason}]`; the rest
--      of the solve still applies.
