-- Accepted board shifts reuse rides; merges release previous solo bookings. REQ §7–8.
  ...
  if v_prop.type = 'deny' then
    ...
    for v_old in select rd.* from public.rides rd join public.ride_requests rr on rr.ride_id=rd.id
      where rr.request_id=v_req.id and rd.status<>'cancelled' and not exists(
        select 1 from jsonb_array_elements(coalesce(v_prop.payload->'legs','[]')) leg where (leg->>'ride_id')::uuid=rd.id
...
begin
  ...
  for q in select id from public.requests where department_id=p_department_id and week_start=p_week_start and requester_id=(select auth.uid()) and status not in ('withdrawn','cancelled') for update loop
