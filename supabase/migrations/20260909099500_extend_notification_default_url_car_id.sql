-- Car care portal: `car_care` notifications carry `data.car_id`, not a request/ride/proposal
-- id, so `notification_default_url()` needs a new mapping before it falls through to the
-- week-scoped branch or `/inbox`. REQ §6 (car care); DATA_MODEL.md §3.11; UX_FLOWS.md §2.1.
-- Reproduced verbatim from 20260909090000_add_notification_default_url.sql with one addition.
