-- REQ §13.77 — submit_request() rejects an ordinary (non-series) new request filed
-- against a week that only exists because a series leg materialized it early (phase
-- 'upcoming'): same `week_not_open` error as a missing `weeks` row. A series leg
-- (`series_id` present, filed by submit_series_request()) is accepted for an `upcoming`
-- week — that is the whole point of the phase.
--
-- Regenerated in full from the live definition (last changed by
-- 20260910093200_extend_submit_request_for_series.sql, not further patched) — the only
-- functional change is the one added `if` right after the week lookup.
