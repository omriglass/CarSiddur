...
export default tseslint.config(
  ...
  // Hard rule 8: src/components/ui/** is generated by the shadcn CLI and
  // never hand-edited — button.tsx/badge.tsx/toggle.tsx export their
  // `*Variants` helpers and form.tsx exports a hook alongside their
  // components, by design, so react-refresh's component-only-export rule
  // does not apply here (documented exception, alongside the Hebrew one
  // just above).
  {
  ...
  // Route-array modules spread into `src/app/router.tsx` (`...adminRoutes`,
  // `...sadranRoutes`, …): the exported arrays are plain data, not
  // components, so these exact names are allowed rather than disabling the
